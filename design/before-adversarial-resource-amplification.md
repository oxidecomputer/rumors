...
corpus; the 2026-07-24 dashu-swap review confirmed the corpus is
already rotted — no seed decodes as a `Clock`, and the
`party_nested` seed decodes as nothing — so the seed-writer and
canonicity check land here as a cure, not a prophylactic), all
workspace snapshots (55 at the worktree baseline: 20
...
- **Evaluate replacing `Base`'s internals with an existing
  ...
  *Landed 2026-07-24 (commit e05e3c4f, this worktree).* What landed:
