...
**A5 (§1.2, byte exhibits; 2026-07-27).** `before`'s skyline topology-flag inversion (flip `40744605`) re-spelled every canonical version encoding after this document's anchor, so §1.2's byte exhibits read against the pre-inversion coding: the empty version, `[0x40]` there, is `[0xC0]` in the re-accepted snapshots, and its two bits `11` now make it the unique lexicographic *maximum* over canonical encodings — no encoding "leads above" it. No election outcome turns on the re-spelling: under the landed `(set_len, bytes)` election (A2, A3) both cited snapshots elect by set size, and both show the bulk side responding, its supplies whole on `Responder stream 0 (height 31)`.
