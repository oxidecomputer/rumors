...
## T170 (2026-09-09): The gossip commit is optimistic; the sink dissolves
Disposes: the p2-commit-path lane's `Discarded` mechanism (its first packet, `14d0d5fb`), re-scoped before merge
Decision: The gossip commit joins the session's result into the live root optimistically: the session's reconciliation already runs against a snapshot, and at commit the critical section swaps the merged root in when the live root is still the snapshot's (pointer identity plus ceiling equality), or releases, re-joins into the moved root outside the lock, and retries; after a small fixed number of failed swaps the committer takes an outer `RwLock` exclusively, every fast-path swap taking it shared, so no committer starves. The local operations keep their walks under the lock, since they mint versions from the tree's ceiling inside the walk and version order must equal commit order for deletion honoring. With every join walk outside the lock, the only value a commit displaces under it is the pre-image root, returned out of the closure and dropped after the guard; the per-node `Discarded` sink, its `#[must_use]` plumbing, and `Tree::discard` dissolve. The property that no user code runs under the replica's lock is upheld without a public statement: a user may do anything in a payload's `Drop` and cannot deadlock the library, an expectation the crate meets without comment; the invariant stays stated at the commit helper for maintainers. The lane lands this on its branch and the packet is rebuilt; the lane merges as one.
Home: lane `p2-commit-path` (re-scope), brief `briefs/p2-commit-path-optimistic.md`.

## T171 (2026-09-09): Improved deterministic meter pins re-pin without confirmation
Disposes: the commit-path re-scope's moved allocation pin (325/3 → 324/1) and, as an instance, the window census's two pins under the optimistic swap; a standing rule for every lane
Decision: A deterministic meter pin that moves in the improving direction (fewer allocations, fewer handles, less fuel, a smaller count, at a measurement the machine cannot perturb) is re-pinned by the lane without Finch's confirmation, with the attribution discipline unchanged: measured at the parent and at the tip, the movement named in the commit message as this change's. A pin that moves the other way, or whose premise the change breaks, is still a finding. Coordinator's application, flagged: the window census's `overhead()` premise (`peak >= before + after`) is broken by an improvement (the join's second copy of the new generation no longer exists at peak), so the lane re-derives the formula to express what the census measures (peak minus the distinct handles across both resting generations) and re-pins under this rule; Finch reverses it in the packet if the re-derivation is not what he wants.
Home: `WORKFLOW.md`'s acceptance rules; lane `p2-commit-path` for the census.
