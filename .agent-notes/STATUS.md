...
Updated: 2026-09-03 04:59 UTC by the rumors session.
...
| p2-link | in review | round 1 landed at `db616f70` (slot returned at one site; two tests; unbiased selects explained), gate clean; `db616f70` verified in full; fresh-eyes round 2 (no bugs) sent exactness items and three test strengthenings as repairs; packet after; stops: the pool-keying shape (four options now, including adapter-owned pooling) and the bound's wording |
| p2-commit-path | packet ready | rebased onto main `1336daa0` (lane diff identical by patch-id), tip compiled and spot-tested; packet `dfa10453` on `triage/p2-commit-path` (rendered HTML in the coordinator's scratchpad); three fresh-eyes rounds, every item landed and verified; one stop (the all-skipped ceiling); its commits unsigned by the lock protocol, re-signed at merge |
