...
1. **`p2-widths`** (tip `e0af2efb`, 51 commits, rulings 33, 34, 39, 41, 104, 109, 110): complete after three repair rounds; the coordinator's acceptance runs all pass; the gate of record was running at the tip when the session paused (`<scratchpad>/p2-widths/coord-gate-final.log`; expect every leg ok but `fuzz`). Packet meta is drafted and filled at `<scratchpad>/p2-widths/meta.md` (acceptance table and rounds written); build with `review.py packet --base main --head before/p2-widths`, render, list in the queue. Two items in its "For Finch's eye" section (the bridge's third door; party-23's table choice). Its one justfile hunk (`fuzz-test`) and `p1-gate`'s justfile edits rebase over each other.
2. **`p2-surface`** (tip `8a2aa4d7`, 44 commits, rulings 42, 44, 46, 47, 48, 49, 98, 104, 105, 109, 110 part 1): complete after three rounds plus a scoped fourth on the visitor, which did not converge in three (recorded in the packet's rounds section as the workflow requires); the coordinator's gate of record at the tip is green but for `fuzz` (`<scratchpad>/p2-surface/coord-gate-final.log`), and the packet is built and listed in the queue (packet head `5315266a`, rendered at the coordinator scratchpad's `p2-surface.html`), with its rendering section (ruling 89) and one stop: the `fuelscape.js` x-axis caption still says "total input bytes". Its registry step (ruling 43) is held for a second launch after `p1-suites`. The coordinator verified every dump and dataset moved only in `size_measure` (two also in `contract`).
