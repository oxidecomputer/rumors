...
- Packets ready, in merge order: `p2-widths`, `p2-surface` (one stop:
  ...
  `p1-gate`, then its children `p8-tagwalk` (packet head `3316c4ed`), `p1-survivors` (packet head `f9ec31d7`), and `p1-fuzz` (packet head `0df4aa4b`, child of p8-tagwalk, before the rumors `p1-collision-mode`), then
...
## Live state of the overnight run (rewritten at every event; last 2026-09-04 00:50 UTC)

| Lane | Branch tip | Agent state | Awaiting | Next |
|---|---|---|---|---|
| p1-board | `f9df45db` (47 commits on main `2fe750ac`; the fourth round: `661b689b` one shared tick scan floor and the ceiling on every tick cell but the id-pair grow cell, `e14803fd` the ceilings re-derived from the governed set by measurement (`TICK_SCAN_CEILING_OVER_FLOOR = 6` from hugeleaf 4.000x, `TICKS_SCAN_CEILING_OVER_FLOOR = 9` from dominated-undercut 6.988x, each x1.25 rounded up to the integer), `4292a444` the span_dominance re-pin under ruling 116, then the probe isolation, docs, taxonomy, three gaps, annotations; the lane's gate: every stream ok but fuzz, the board stream green for the first time) | the scoped re-read landed: a round-3 test deleted undisclosed by the probe rewrite (`rank_declaration_matches_the_operand_on_every_board_family`, verified gone at the tip), the seed-ticked cells' limb and touch floors not-applicable over live work (the kernel's consuming max scan), the tick ceiling 6 where its rule gives 5, ruling 117's doc sentence (the coordinator's) false and corrected in the rulings record, "every cell" overstatements, no wiring check, one-sided probe arms. A fifth scoped round is with the lane agent (23:58 UTC message; nine items); coordinator acceptance at `f9df45db` was green but is superseded | the lane's round-5 report | coordinator acceptance at the new tip, a scoped re-read of round 5, then the packet |
