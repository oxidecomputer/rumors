...
1. **`p2-widths`** (code tip `e0af2efb`, packet head `ba36e970`, 53 commits on `f7703c4f`; rulings 33, 34, 39, 41, 104, 109, 110): REOPENED under ruling 118 (2026-09-04): the narrow-or-wide fold table (party-23) is dissolved into one `Vec<u64>` and the log-factor liveness floor re-derived; the lane is audited for the same pattern. The earlier packet (`ba36e970`, rendered) is superseded when the repair lands; the lane agent's report, then the coordinator's acceptance and a scoped review, then the rebuilt packet. Two items in its "For Finch's eye" section (the bridge's third door; party-23's table choice, now reversed by 118). Its one justfile hunk (`fuzz-test`) and `p1-gate`'s justfile edits rebase over each other.
...
## Live state of the overnight run (rewritten at every event; last 2026-09-04 04:30 UTC)
