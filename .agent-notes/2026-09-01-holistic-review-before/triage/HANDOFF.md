...
## Live state of the overnight run (rewritten at every event; last 2026-09-03 05:25 UTC)
...
| p1-fuzz | `/Users/oxide/src/before-p1-fuzz` at `fdd47747` (stacked on the old gate tip) | running; stops before its calibration and reports | its pre-calibration report | then `git rebase --onto 95377663 fdd47747 before/p1-fuzz` (the tagwalk tip, itself on `90f7dd9c`), then tell it to calibrate once; then acceptance, rounds, packet |
| p1-survivors | rebased onto `90f7dd9c` (four commits: codec-bits-15, skyline-watermark-24, suanpan-40, annotations; the gate read red only on the three inherited items) | running: landing suanpan-tests-25 in ruling 88's form (ceilings plus the four-touches-per-round floor; the brief's exact grid was ruling 50's older form) and converting its two new exact pins to the same form | its final report | acceptance re-runs, one fresh-eyes round, packet; child of p1-gate |
| p8-tagwalk | on `90f7dd9c` (`9f9263b2` the tag pair, `95377663` annotations, plus a small repair round landing: the `live < 2` guard cases, four doc items) | running the repair round; the fresh-eyes review found no defect | its round report | re-run the codec filter at its tip, packet; then p1-fuzz rebases onto its final tip and calibrates |
