...
## Live state of the overnight run (rewritten at every event; last 2026-09-03 05:04 UTC)
...
| p1-gate | `90f7dd9c` (37 commits on main; a fixup rebase rewrote everything from the convergence commit up, so the three children that started at `fdd47747` are rebased onto this tip with `git rebase --onto 90f7dd9c fdd47747 <child>` when each reports; the ten commits from the coordinator's rebase and the seventeen from `cc3facf2` up are unsigned, re-signed at merge) | second round landed; idle | the coordinator's acceptance run (in flight) | fill the packet's table, build, render, queue with stop 1 (the syn roster); no third round |
