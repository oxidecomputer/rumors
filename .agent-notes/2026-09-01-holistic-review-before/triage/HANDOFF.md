...
2. **`p2-surface`** (code tip `85a735cb`, annotations `092ad7b3`, on `cad0b75d`; rulings 42, 44, 46, 47, 48, 49, 98, 104, 105, 109, 110 part 1, 119): the ruling-119 relabel landed (one commit, 223 files: the roster's eleven measure constants and every literal become `INPUT_BYTES`, the row knowledge as `//` comments, 104 datasets and 104 dumps relabeled in the one field, the island summary and noscript, the widget's caption and readout, and the SVG axis and stamp all derived from the label; the deleted `capped_size_measure_names_the_guest_cap` test; the lane's gate clean but for the fuzz port); coordinator acceptance green at the tip (`p2-surface/coord/acceptance-092ad7b3.log`: both compiles, fuelscape-test 42 passed, fuelscape-verify re-derives 104 datasets and diffs clean; meta updated) and the scoped fresh-eyes review of the relabel landed ready (all 104 datasets and dumps verified by script; four small items sent back to the lane agent, 05:50 UTC); on its report: one `fuelscape-test` re-check, then the packet is rebuilt (`review.py packet --base d5085a58`), re-rendered, and re-queued. For Finch: `AtlasData.unary` now dead (a format bump to remove; `new-findings.md`); the "packed bytes" sites in `crates/before/src` routed to the prose pass. Its registry step (ruling 43) is held for a second launch after `p1-suites`.
...
## Live state of the overnight run (rewritten at every event; last 2026-09-04 05:52 UTC)
