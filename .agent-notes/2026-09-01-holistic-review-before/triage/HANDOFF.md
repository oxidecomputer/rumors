...
2. **`p2-surface`** (tip `8a2aa4d7`, 44 commits, rulings 42, 44, 46, 47, 48, 49, 98, 104, 105, 109, 110 part 1): complete after three rounds plus a scoped fourth on the visitor, which did not converge in three (recorded in the packet's rounds section as the workflow requires); the coordinator's gate of record was running at the tip (`<scratchpad>/p2-surface/coord-gate-final.log`). Packet meta at `<scratchpad>/p2-surface/meta.md` with its rendering section (ruling 89) and one stop: the `fuelscape.js` x-axis caption still says "total input bytes". Its registry step (ruling 43) is held for a second launch after `p1-suites`. The coordinator verified every dump and dataset moved only in `size_measure` (two also in `contract`).
