...
## Live state of the overnight run (rewritten at every event; last 2026-09-03 04:20 UTC)
...
| p1-fuzz | worktree `/Users/oxide/src/before-p1-fuzz` at `fdd47747` (stacked on p1-gate) | not launched: held for box load (186 at 04:19 UTC; a watch in this session fires under 150; after a compaction, check `uptime` and launch when under 150) | the load | launch with its brief plus: the band refit under ruling 14's one calibration (the seeds from the gate lane's commit 47544320 and the generators lane's scratchpad files; the stale-pin cause 5d167a63), the guest-closure provenance pin, the `call1_with_panic` channel, the widths lane's `fuzz-test` recipe already on that branch; overnight rules |
| p1-survivors | worktree `/Users/oxide/src/before-p1-survivors` at `fdd47747` (stacked on p1-gate) | not launched: held for box load with p1-fuzz | the load | launch with its brief; overnight rules |
