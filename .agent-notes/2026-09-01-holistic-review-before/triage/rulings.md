...
## Ruling 107 (2026-09-02): the rumors plan's gate lane owns the shared root files in wave 1; before stacks on it

Disposes: the cross-plan placement of gate-legs-6, gate-legs-2, deps-6, surface-roster-1 (rulings 18 and 25) against the rumors triage's `triage/p1-gate` lane, which edits `ci.yml`, the justfile header and `ci` recipe, the root lockfile, and `tools/testdoc` before any before lane has launched.

Finch's words: "Ruling 18 governs, workspace-wide"; "Stack on rumors' triage/p1-gate now"; "Keep `just --evaluate`; before's lane replaces the sed."

Decision. (1) Ruling 18 governs the whole workspace: the mutation roster, `tools/mutantcheck` and its expected file, the `mutants-list` leg, and the cargo-mutants CI install retire even though the rumors gate lane's commit `c35943ca` pins `cargo-mutants@27.1.0` and describes the roster's counts as a committed expectation. That commit lands as its lane wrote it; before's gate lane deletes the pin and restates the comment on top. (2) Before's gate lane branches from the tip of the rumors lane branch `triage/p1-gate`, its packet is diffed against that branch, and it never merges before that lane does; when Finch's review moves that branch, the coordinator rebases and range-diffs. (3) The mechanism of ruling 25 stands: every CI nightly derives from `just --evaluate nightly_toolchain`, `just` installed first as a prebuilt binary. The rumors lane's `sed` derivation is replaced, not kept; which lane replaces it is settled between the two coordinators before before's gate lane launches (the rumors lane doing it once, before its packet, is the cheaper shape and the one proposed to that session).

What follows. Finch authorized the two coordinator sessions to message each other directly and resolve cross-plan conflicts between themselves, with every exchange visible to him in both sessions. The shared rules proposed to the rumors session (one owner per shared root file per wave; root-file edits as a lane's own last commit; the retiring lane goes second when one plan hardens an instrument the other retires; a merge queue at `.agent-notes/merge-queue.md`; explicit-pathspec commits and no rebase, checkout, stash, or reset in the primary worktree; four building lanes per session on the box and one wall-time measurement at a time) are recorded in `triage/WORKFLOW.md` once agreed. `tools/digestshare` (ruling 79) and `tools/workflowlint` (ruling 106) are deleted only after the rumors lanes that edit or cite them have merged.

Outcome (same day). The rumors session agreed: its gate lane adopts the `just --evaluate` derivation and drops the cargo-mutants install and pin before its packet is built, so `ci.yml` has one owner in wave 1 and before's gate lane verifies rather than edits those hunks (gate-legs-2, deps-6, surface-roster-1 are recorded as landed by that lane's commit once it merges). Its amendments to the shared rules are adopted in `.agent-notes/merge-queue.md`: a lane whose whole surface is root files commits per entry and stacks, rather than isolating a last commit; rewrites of `main` happen at Finch's word with the other session paused and are announced in the queue first; publication prep (rumors T79) runs last as one joint lane. The rumors gate branch's tip moves at least twice more (a rebase onto its memwatch lane, then review repairs); `before/p1-gate` launches after the first move.

Second outcome (same day). Two fresh-eyes findings from the rumors gate lane amend the above. (a) `just --evaluate` needs a toolchain in this workspace, because the justfile's `host_triple` is a backtick assignment that runs `rustc` and `just` evaluates every assignment before anything else; the derivation therefore installs the stable pin first (read from `rust-toolchain.toml` with `sed`, which needs nothing), then `just`, then evaluates the nightly. Ruling 25's mechanism for the nightly is unchanged; the stable pin's read is outside it. (b) The cargo-mutants install stays in the rumors gate lane, pinned at the version `tools/mutantcheck-expected.json` names, because the `ci` roster names `mutants-list` until before's gate lane removes it and a workflow naming a tool it does not install is red at every commit in between; before's gate lane deletes the install, the leg, and the roster line in one commit, as decision (1) first said.


## Ruling 108 (2026-09-03): the meter suite's feature guard is `required-features`; the rank twins retire; the wide-arming render row lands

Disposes: meter-adequacy-11 (amending ruling 51); the harness lane's two findings outside its entries (the `RANK_*` and `SKYLINE_RANK_*` twin pair; the `WideArming` render coverage dropped by folding `skyline_render_records_zero_touches`).

Decision. (1) Ruling 51's `compile_error!` also fails `just check` and the gate's default-feature clippy leg, since both compile the test target under default features. The mechanism is instead a `[[test]] name = "meter"` entry in `crates/before/Cargo.toml` with `required-features = ["limb-meter", "scan-meter"]`, the crate's existing idiom for `amp_board`: an explicit `--test meter` without the features is a cargo error naming them, blanket `check`, `clippy --tests`, and `nextest --workspace` runs skip the target, the gate builds it under `--all-features`, and `just test` passes the features for `-p before` so the inner loop runs the suite. The suite's `cfg(not(feature))` read arms, which existed only for the guard's absence, are deleted with it. (2) `SKYLINE_RANK_DENSE`, `SKYLINE_RANK_BIGROOT`, and `SKYLINE_RANK_HARMONIC` pin the same shape and operation as the public `RANK_*` rows with identical readings (`Version::rank` is `query::rank` on the live skyline); they retire under ruling 4's rule that the public entry is the roster of record, the door rows carrying the twins' ceilings where those are tighter. (3) A `SKYLINE_RENDER_WIDE_ARMING` row is added in the harness lane, pinned from a parent measurement, so the render side of the wide-arming dual stays covered.

Outcome (rulings 4, 38, 50, 51, 108 for the harness lane). Landed on `main` at `5b0a17d4` (lane `p1-harness`), merged at Finch's word after his review of the packet; the ledger carries the per-entry shas.

## Ruling 109 (2026-09-03): no proptest sets its case count explicitly

Disposes: the before-side half of the rumors ruling T148 (CI runs the suites under the release profile with `PROPTEST_CASES` set to a large measured number); the rumors session asked whether its bypass check should be workspace-wide from the start.

Finch's words: "Just don't explicitly set the cases ever. Then it's trivial."

Decision. No `ProptestConfig` anywhere in the workspace sets `cases`; every property test runs proptest's default count and takes `PROPTEST_CASES` from the environment as proptest itself does, so no helper is needed and the committed check reduces to the absence of an explicit `cases` in any `ProptestConfig`, workspace-wide from the start. The explicit sites in `crates/before` and `crates/suanpan` are removed in a small before lane based on `main` (`p1-proptest-cases`), with any site whose count was raised or lowered for a stated reason reported rather than silently defaulted.
...
Finch's words on crate-root-6: "Yes, change to its own dataset measure, but uniformize the language while you're at it. 'Packed bytes' means nothing; there is no separate 'packed' representation, as you can see."

Decision. (1) Every fuelscape island's rendered summary and noscript text states the denominator its dataset records, derived from the dataset's `size_measure` at render time, never hand-written per island; and the measures' vocabulary is made uniform across the roster rows and the committed datasets: the operands' encoded bytes, the crate's own term for what `encode` produces, with no "packed" anywhere. This is a deliberate rendering change under ruling 89; the packet shows panels before and after, and the coordinator names the uniform phrase it chose for Finch's eye. (2) The link-index widening lands with its cost committed as the enforced baseline, per the instruments-before-cures rule: the two meter rows and the board's mirror-narrow tick trio are re-pinned at the widened readings, each attributed in the commit as heap paid for the correctness fix with the cure pending in `p2-cures`, which tightens them back; nothing is declared a model, and the gate is green. (3) The version composer builds in one pass through a sealed, doc-hidden `VersionLiteral` trait for `u64` and `(u64, T, S)` leaves, the public impl's bound moving from `where Version: TryFrom<T, Error = Parse>` to `T: VersionLiteral`, the same shape and argument as the party composer; landed with clock-14's flatness pin in one commit. (4) The oracle bridge offers asserting doors (normal, non-empty) for the differential suites and explicitly named raw doors, documented as existing to lower malformed trees, for the reject tests; instrument surface only. (5) party-23: the lane chose the narrow-or-wide table over the unconditional `u64` one because the ruling's heap criterion was silent while a committed log-factor pin moved under the unconditional table; the coordinator accepts the choice as the goal-over-mechanism resolution and records it here for Finch's eye.

## Coordinator note under ruling 78 (2026-09-03): meter-adequacy-9's judge half is moot

meter-adequacy-9 (a low, roster approved by ruling 104) asked the bench harnesses to write the denominator sidecar after the run and the bench judge to refuse medians older than it. Ruling 78 retires the bench judge, its sidecars, and the A/B arms (`p5-judge`), so the judge half builds what that lane deletes; the `p1-gate` brief carried the row past the ruling. The judge half is `dup` of ruling 78's retirement and lands nowhere; the gate lane drops its commit for it before review. The row's other half (a build identifier exported by the wasm guests and asserted by their harnesses) stays with `p1-fuzz`, as the brief already hands it.

## Ruling 111 (2026-09-03): generators draw their constraints; `prop_assume!` only where a constraint cannot be generated

Disposes: the reject-cap finding on `version::tests::grow_matches_brute_force` and `grow_minimal` (new-findings.md), and every other `prop_assume!` in `crates/before` and `crates/suanpan`. Relayed by the rumors session as its ruling T157; Finch's words, through it: "global rejections being too many should be disposed by fixing generators to be more generative; please don't do prop_assume unless it's absolutely necessary."

Decision. A property test's generator produces values that satisfy the test's constraints; `prop_assume!` remains only where a constraint cannot be generated, with the reason stated at the site. The seven sites in before (`oracle/tests.rs:598`, `version/tests.rs:560` and `:588`, `clock/tests.rs:1135`, `:1138`, `:1141`, `:1213`) are swept by a small lane, `p2-generators`, from `main`: each becomes a generator that draws the constrained value (an overlay the fill fixes; a non-empty candidate set; an encoding that fails to decode, drawn as a corruption of a valid one) or keeps its `prop_assume!` with its reason. The rumors P5 tests lane's lint-tier check holds the workspace's surviving sites to the named ones, before's included.

## Coordinator note under ruling 110 part 4 (2026-09-03): the bridge's third door

Ruling 110 part 4 gave the oracle bridge asserting doors (normal, non-empty) for the differential suites and named raw doors for the five reject tests. The widths lane found the non-emptiness assertion refused a dozen legitimate callers that lower the anonymous region as an algebra operand (the fill, grow, and query exhaustive and family-pair suites, the generators, the descriptor drivers, two party algebra proptests), whose own docs admit the empty region. The lane added a third door, `from_oracle_region` (normality asserted, emptiness admitted, documented as the algebra operands' door), keeping raw for malformed trees. The coordinator accepts it as the goal-over-mechanism reading of the ruling (asserting doors for legitimate use; raw only for malformed input) and records it here for Finch's eye at the packet; he may rule the alternative (raw for all of them).

## Ruling 112 (2026-09-03): the box syncs through git; the coverage reproduction waits for CI; the lock convergence is the shipped closure

Disposes: the gate lane's three stops (the supply-chain recipe on a checkout-less box; gate-legs-1's venue; deps-9's scope).

Finch's words: "Fix the illumos wrapper skill so it instructs you to use git to sync to the box, then do that." "I will wait for this to happen in CI again. Until then, I don't care." "Accept the shipped-closure scope."

Decision. (1) Ruling 16's derivation from `git ls-files` stands untouched; what changes is the box: the `building-on-illumos` wrapper syncs a worktree to the box through git, so the synced tree is a checkout and every recipe that asks git a question runs there as it does on the Mac. The coordinator amends the skill and its wrapper, then the lanes use it; the gate of record for the gate lane is re-run under it. (2) gate-legs-1 (ruling 24's two-run reproduction of the coverage leg's red) is deferred to CI: the next time the coverage job reads red on a byte-identical tree, that run is the reproduction, and the remedy follows ruling 24 unchanged; until then no heap pin under coverage moves for that reason, and nothing else waits on it (the ordering clause in the briefs that made P2 wait on gate-legs-1 is lifted). The ledger row reads `defer` with this ruling as its home. (3) deps-9's convergence is `before`'s shipped dependency closure (dev edges excluded) resolving identically across every committed lock, held by `tools/lockcheck`; tool-only crates may differ.

## Ruling 113 (2026-09-03): no proptest strategy rejects

Disposes: widens ruling 111 and the `p2-generators` lane. Relayed by the rumors session as its ruling T161; Finch's purpose in his words, through it: "Then we can run them as long as we want."

Decision. No property-test strategy in the workspace rejects a draw: no `prop_assume!` (no survivors, no reasons at the site), no `prop_filter` or `prop_filter_map`, and no `hash_set`, `btree_set`, `hash_map`, or `btree_map` strategy with a nonzero minimum size (they reject internally on collisions; `sample::subsequence` is the total form). Every constraint is generated. Enforcement in two layers, both the rumors plan's to land: a lint-tier check with no allow list (`tools/caselint`, the four spellings, its roots widening to `crates` once before's sweep merges), and the gate's test recipes and the CI jobs exporting `PROPTEST_MAX_GLOBAL_REJECTS=0 PROPTEST_MAX_LOCAL_REJECTS=0`, so any rejection aborts its property at the default count. Before's `p2-generators` lane widens to its seven `prop_filter` sites (the fuzzfit program strategy, two in `codec/tests.rs`, two in `borsh_impls/tests.rs`, one in `testing/generators.rs`, one in `version/skyline/query/tests.rs`); it has no collection strategy with a minimum size and no `prop_assume!` left. Its packet is rebuilt over the widened lane before Finch reads it.

## Ruling 114 (2026-09-03): the lock convergence goes upward; bytes 1.12.1 everywhere

Disposes: the gate lane's fuel-band stop (deps-9's sweep and the fuzzfit sentry).

Finch's words: "Are you sure that was due to dependency changes? What would have changed it? Root-cause it." Then, on the control showing the sweep's parent green and the tip red with bytes moved from 1.12.1 to 1.11.1: "Wait, why don't we upgrade Bytes and recapture the win?"

Decision. The deps-9 sweep converged the fuzzfit guest's lock downward onto the root lock's older `bytes` (1.11.1) while every detached lock, and the fuel bands' calibration, had 1.12.1; the guest's `Party::decode` then read about 1.9x its band. The convergence goes the other way: the root lock takes `bytes` 1.12.1 (the manifests already admit it) and the detached locks agree, so the whole workspace takes the win the bands had measured; no band moves. The lane still owes the mechanism (what in `bytes` changed the decode path's cost), and hands `p1-fuzz` a provenance pin for the guest's linked closure so a lock-only move of a crate the guest executes reads red by name. Root `Cargo.lock` is a shared root file; the touch is announced in the merge queue.

Coda (same day). Finch's words: "Quite so; indeed, we should generally speaking be upgrading our dependencies when possible, in general." The rule generalizes past `bytes`: when locks are converged, the target is the newest version any committed lock resolves within the manifests' requirements, and the root lock is brought up to it, never a detached lock down; more generally, dependencies are upgraded when possible, and a lockfile edit that lowers a version is a finding to report, not a convergence. The deps-9 sweep applies this to every crate it moved (memchr, thiserror, serde and its derive, serde_json, zmij, proc-macro2, quote, rustversion, syn), and `tools/lockcheck`'s docstring and the supply-chain recipe state the direction.

## Ruling 115 (2026-09-03, night): the id tag walk gets its cursor before the bands are refit

Disposes: the `BitsView::bit` cost on the id walk (new-findings.md; the gate lane's bisect to `5d167a63`); the order of the fuzz lane's calibration.

Finch's words: "Go for it. I don't think it changes the execution time meaningfully. I think this should be a pretty quick fix, no?"

Decision. A small lane, `p8-tagwalk`, stacked on the gate lane's tip beside `p1-fuzz`, gives `IdReader` a `usize` byte-and-offset cursor so a node's two tag bits are read with one shift and mask and one bounds check per node, the access pattern bitvec's slice had, on the owned representation; the recorded bit position stays the reader's interface. It is a strict deletion of redundant work (fixed sign): constructed, then measured on the fuzzfit harness's per-bit fuel for `ff_party_decode` at the parent and after, on the seeded fork-chain program and the calibration corpus, with the meter suite's id rows checked for any moved reading. The fuzz lane holds its one calibration until it has rebased onto this lane's tip, so the bands are pinned once against the tree that carries the recapture.


## Coordinator note under ruling 88 (2026-09-03, night): suanpan-tests-25 lands in ruling 88's form

The `p1-survivors` brief quotes ruling 50's Resolution for suanpan-tests-25 (an exact grid, `16n + 2` at four cells), and the coordinator's launch message repeated it; ruling 88, later and naming the entry, retired suanpan's exact-touch contract: the grid is pinned as ceilings plus the universal four-touches-per-round floor. The ruling is the authority when a brief disagrees, so the lane lands ruling 88's form, and the two exact pins it added under suanpan-40 (ruling 18's wording predates 88) take the same form; the file's pre-existing exact pins are `p6-suanpan`'s wholesale conversion. The gloss in the launch message was the coordinator's error, recorded here.
...
## Ruling 118 (2026-09-04): the fold index is one `Vec<u64>` table; instruments never shape the code

Disposes: party-23's table choice (ruling 110 part 5, reversed); a lane-wide audit of `p2-widths` for the same pattern.

Finch's words: "This seems overly contrived, and the justification seems to say it's trying to prevent a *drop* in a liveness floor? This is circular, I think. Recall our goal was to *simplify* the codebase." Then: "Dissolve it all, as you suggest. Please also identify if anything else in this lane did similarly."

Decision. The fold index's right-child table is an unconditional `Vec<u64>` at every operand size: the `TableWord` trait and its two impls, the `Rights` enum, the `narrow` predicate, the test-only `build_wide` door, and `probe_scan_bits` are dissolved, and one build pass and one walk remain. Ruling 33's own criterion for that shape (the board's `party_join_all` heap reading tolerating the wider table, measured at the parent) was met by the lane's measurement (0.0 to 0.2 bytes per byte on weave, nothing elsewhere); the narrow-or-wide table was chosen only to keep the `party_join_all` log-factor liveness floor (a ratio of metered scan bits whose committed value assumed 32-bit probes) from reading under its threshold. That is the instrument owning the design: a legitimate change that falls below a floor is a finding about the floor's premise, so the floor is re-derived under the 64-bit probe currency, at the parent, with the movement attributed in the commit, and every probe records 64 bits. The coordinator's acceptance of the choice in ruling 110 part 5 was an error and is reversed. The whole `p2-widths` lane is audited for the same pattern (code shaped, widened, split, or made generic so that a committed pin, floor, or measured reading would not move, where the ruled change legitimately moves it); each instance is reported with its evidence for Finch to rule, not repaired unbidden.

## Ruling 119 (2026-09-04): a size measure is a unit label

Disposes: ruling 110 part 1's wording; the surface packet's stop on the `fuelscape.js` x-axis caption; every `size_measure` string in the fuelscape roster and the committed datasets.

Finch's words: "I agree that the units should be consistent, but the ones you've chosen are also too verbose. There's no distinction between 'encoded' bytes and those in memory; they are the same bytes. Please ensure captions are minimal and concise. The x-axis caption can use the same units. Just please make sure they are as uniform and concise as possible, and do not use undefined jargon." Then: "size measure should not be 4 lines of text with parentheticals!!!"

Decision. `size_measure` is the unit the island's axis and summary print, a few words at most, identical wherever the unit is the same: in this roster every row's size is the bytes of its inputs, so every row reads `input bytes` and nothing else ("encoded" goes: the bytes in memory are the encoded bytes). The x-axis caption reads `input bytes (log scale)`. How a row draws its inputs (the split across operands, the arity law, what is composed in unmeasured preparation, declared constants) is maintainer knowledge and moves to the row's rustdoc, never into a rendered string; the render derives the island's summary and noscript text from the label with one uniform sentence. The committed datasets' recorded `size_measure` fields take the same label (a metadata edit, no survey; the measurements are unchanged). The coordinator's earlier phrase, "the operands' encoded bytes", was the wrong altitude and is withdrawn.

## Ruling 120 (2026-09-04): the surface census and the hand roster dissolve into `cargo public-api`

Disposes: the surface roster and census as instruments (`crates/before/surfacecheck`, `crates/before/src/surface.rs`'s `METHOD_SURFACE` and `FAMILY_SURFACE`, `testing/surface_coverage.rs`, the `surface-scan` extractor, the `surface-totality` gate leg and its rustdoc-JSON build, the `rustdoc-types` pin and its bump procedure); re-reads rulings 40, 47, 49, and 98's replacement half; opens a lane, `p2-census`.

Finch's words: "what hazard is the surface census actually supposed to prevent? Do we need this instrument?" Then, on the coordinator's recommendation: "Great, please do exactly your recommendation. Dissolve all the hand-tooling."

Decision. The hazard the census guards, a public item appearing, disappearing, or changing without review, is guarded by a maintained tool: `cargo public-api` enumerates the crate's public surface from rustdoc JSON (inherent functions, trait impls, auto-trait impls, hidden items) and diffs it against a committed text snapshot. The snapshot is committed under `crates/before/` and diffed in the gate; a public item entering or leaving reads red until the snapshot is deliberately re-accepted, under the same discipline as the wire snapshots (a re-accepting commit names the API change). The tool's version is pinned where the toolchain pins live, with the rationale and the bump procedure, since the snapshot is compared textually. Every hand-maintained enumeration of the public surface dissolves: `surfacecheck`, the `syn` extractor, `METHOD_SURFACE`, `FAMILY_SURFACE`, the coverage suite's citation machinery, and their gate and CI legs. The two consumers of the roster, fuelscape's coverage table and the fuzzfit harness's exemption tiling test (ruling 14), derive the surface from the tool's snapshot through one small adapter (a parser of the snapshot's lines, or the rustdoc JSON directly), never from a hand list. Ruling 49's auto-trait pins are carried by the snapshot. Ruling 98's per-public-item coverage floor is deferred to when coverage runs in CI (ruling 112) and is recorded as open. The lane stacks on `before/p1-fuzz`'s code tip, since the tiling test lives there; its rebase over the merged surface lane resolves every conflict toward deletion.

## Ruling 121 (2026-09-04): ruling 120 extends to suanpan; `surface-scan` dissolves

Disposes: ruling 78's clause keeping suanpan on the `syn` extractor; ruling 47's parser as a crate; suanpan's `FAMILY_SURFACE`.

Finch's words, on the coordinator's recommendation that suanpan's claims roster derive its public functions from a `cargo public-api` snapshot of suanpan and its test names from `cargo nextest list`, retiring `surface-scan` entirely: "Yes, of course it should be extended."

Decision. The `surface-scan` crate dissolves with its workspace entries. suanpan's claims roster keeps its table-to-roster binding (the crate page's cost cells against the claims, mechanical and about the page) and derives its totality leg from a committed `cargo public-api` snapshot of `suanpan`, rendered by the same recipe and diffed by the same gate leg as before's, through the same adapter (shared between the two crates by one mechanism, never two copies); suanpan's `FAMILY_SURFACE` dissolves into the adapter's derived operations set, exactly as before's did under ruling 120's first review. The claim-to-evidence leg stops scanning source text: a cited witness's existence is checked against `cargo nextest list`'s output (a maintained tool), never against a text scan of test files. Coordinator's note for Finch: ruling 98's reasoning, that nothing can check a cited test exercises the item it is cited for, applies to suanpan's citations as much as it did to before's; the coordinator recommends they dissolve under the same regime when the coverage floor lands (ruling 112), and until then the existence check is the honest minimum. The work lands in the `p2-census` lane.

## Ruling 122 (2026-09-04): the operations tiling dissolves; the methods tiling stays

Disposes: the operations set added under ruling 120's first review; the per-impl entries in fuelscape's covers table, the board's coverage table, and suanpan's totality; the second review's findings on their false dispositions.

Finch's words: "What is the operations tiling used for? Can it be entirely dissolved? What would be the impact of doing that?" Then: "Go for it. Especially because the public API enumeration is checked against those other tables, right?"

Decision. The trait-impl half of the coverage tilings dissolves: the adapter's derived operations set, `Surface::tiled()`, the impl-line entries in the two `before` tables and in suanpan's roster, and their planted-operation controls. Each consumer holds the snapshot's inherent methods total (priced or excused by name, red on a method with neither), as it did at the base and as ruling 14 rules for fuzzfit; the snapshot diff in the gate is the review point for operator and conversion impls, whose cost claims live in rustdoc's `# Complexity` sections. Every measurement stays. The module docs say "every public method", not "every public operation". The second review's lows land with it: witness resolution by full test path, the tables' wording, `Surface::names` narrowed to what a covers entry may cite.

## Ruling 123 (2026-09-04): the coverage tables declare only the exceptions

Disposes: fuelscape's `covers` fields, the board's `BOARD_PRICED` table, fuzzfit's `Op::covers` mapping; narrows rulings 14 and 122's "priced or excused by name".

Finch's words: "Could we record only the exceptions list, and skip the enumeration of the ones that are covered? This minimizes what needs to be declared."

Decision. Each cost instrument's covered set is derived from its own roster of what it runs, never declared: an op, panel, or row name in snake form (`version_tick`, `party_join_all`, `ff_rank_decode`, a variant suffix stripped by a stated rule) resolves mechanically to a `Type::method` or `module::function` in the snapshot, and the tiling holds that every snapshot method either resolves from some op's name or is on the instrument's exceptions list with its reason, and that every op name that resolves names a method that exists. What is declared is the exceptions list (the decision record: what is deliberately unmeasured and why) and the few rows that measure no public method by design (controls, adversarial families), which fail by name if they resolve to nothing and are not so marked. The naming convention is the claim, stated once at each roster's doc; a misnamed op is visible beside its own measure. suanpan's claims roster keeps its rows: they are the claims the crate page is held to, not a covered enumeration.
