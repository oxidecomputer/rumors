...
1. **The front-page bound** (crate-root-25, paper-fidelity-3, crate-root-40). Is the intended headline "linear for the core operations (tick, fork, join, meet, comparison, the codecs), near-linear for the n-ary folds, multiplication-bound where the answer is a wide integer"? The constructed rows answer the second question the entries raise, whether any operation is over its documented bound today: five are, so the meter header's "far from" is true of them. Recommendation: reword the headline to the per-operation contracts and name the five demonstrated rows in this document (join re-anchor, `Ranked::cmp`, masked `peek_flip`, coverage per-hole sweeps, memo heap) as today's exceptions to `lib.rs:350-353`, in `lib.rs` or the meter header, until each is fixed or given an owner-declared model; `just amp-board-acceptance` cannot settle which sentence is stale, because its committed families are the shapes on which the claims hold, and the constructed rows already have.
...
3. **Join's re-anchor cascade** (skyline-coding-9): cure with the two-stream builder (topology and payload interleaved once in `finish()`, dissolving the held leaf, `lens`, and `extract_code`), or accept `Θ(output + Σ re-anchored widths)` and restate the join/meet/span contracts? Recommendation: cure; `O(|self| + |other|)` for join is the crate's headline guarantee. Land the two-scale scan pin on `join(Hugeleaf, spine_of_pairs(d))` red first.
