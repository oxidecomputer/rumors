...
Sush targets Helios; macOS execution is not a requirement. Run Sush builds,
Clippy, and behavioral tests on `ox-east-1-agent`, using the
[building-on-illumos skill](/Users/oxide/.claude/skills/building-on-illumos/SKILL.md).
Do not add macOS compatibility fixes or treat Mac-only failures as blockers.
