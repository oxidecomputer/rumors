...
use super::{SPEAKERS, closing_frame, encoded};
...
async fn routes_all_streams_and_preserves_the_trailing_boundary() {
    ...
    for speaker in SPEAKERS {
        let frames = (0..STREAM_COUNT).rev().map(|index| {
        ...
        for index in 0..STREAM_COUNT {
            ...
            assert_eq!(frames.next().await, Some(closing_frame(speaker, stream)));
...
async fn reports_every_stream_left_open_at_eof() {
    for speaker in SPEAKERS {
        ...
        let bytes = encoded(speaker, [(closed, closing_frame::<()>(speaker, closed))]);
...
async fn rejects_a_frame_after_stream_end() {
    for speaker in SPEAKERS {
        ...
        let frame = closing_frame::<()>(speaker, stream);
        let bytes = encoded(speaker, [(stream, frame.clone()), (stream, frame)]);
        let (demux, _incoming) = build_incoming::<_, ()>(speaker, bytes.as_slice());
        assert!(matches!(
            demux.run().await,
