...
//! greedily up to the [`RunBudget`], a run never dips below one record, and
//! a run never spans reactions. Round trips are asserted through the same
...
/// whole case's run, so the sweep covers zero, sub-record, mid-run, and
/// run-swallowing budgets.
const MAX_CASE_BUDGET: usize = (MAX_CASE_LEAVES + 1) * MAX_RECORD_LEN;
...
proptest! {
    ...
    ///
    /// Records accumulate in path order until the next would overflow the
    /// budget; every run flushed early is full (its successor's first
    /// record would not have fit); no run is empty; only a lone record may
    /// exceed the budget. Decoding the chunked frames reproduces the exact
    /// leaves, so chunking is invisible above the wire.
    #[test]
    fn supply_runs_chunk_greedily_by_bytes(
        ...
        for (position, run) in runs.iter().enumerate() {
            ...
            prop_assert!(
                run.encoded_len() <= budget || records == 1,
            ...
            if position + 1 < runs.len() {
                let (version, message) = runs[position + 1]
                    ...
                    .expect("an encoder-produced run holds canonical records");
                prop_assert!(
                    run.encoded_len() + LeafRun::record_len(&version, &message) > budget,
