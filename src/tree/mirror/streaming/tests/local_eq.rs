...
proptest! {
    ...
    fn view_projection_is_sound(spec in arb_divergence()) {
        ...
        prop_assert_eq!(
            ...
            "equalized remote set sizes and ceilings fix the local role \
             across sessions"
        );
...
fn free_insertions_are_invisible_to_the_local_view() {
    ...
    for case in 0..CASES {
        ...
        // Each remote gets one extra subtree at radix 7 under a scope the
        // local party answers, so the new R child is invisible to its view.
        ...
        let join = ceiling_of(&probe);
        let local = rooted(local_node);
        let p = client_role(&local, &rooted_at(probe, join.clone()));
        // Two scopes the local party answers, at different depths of the
        // anchored disputed chain: p = I answers the odd heights [0]
        // (h31) and [0, 0, 0] (h29); p = R answers the even heights []
        // (h32) and [0, 0] (h30). Different depths make the two sessions'
        // skeletons differ structurally.
        let (shallow, deep): (&[u8], &[u8]) = match p {
            Party::I => (&[0], &[0, 0, 0]),
            Party::R => (&[], &[0, 0]),
        };
...
fn leaf_requests_are_erased_from_the_view() {
    ...
    // outright.
    {
        ...
        let local = rooted(base);
        assert_eq!(client_role(&local, &remote_0), Party::I);
    ...
    // byte tiebreak.
    {
        ...
        let (remote_0, remote_1) = (0..TIEBREAK_ATTEMPTS)
            ...
            .find(|(_, remote_1)| client_role(&local, remote_1) == Party::R)
            .expect("some tiebreak inflation makes the local side respond");
        assert_eq!(client_role(&local, &remote_0), Party::R);
        check(
            local,
            remote_0,
            remote_1,
