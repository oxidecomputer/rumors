...
fn overlapping_retiree_party_is_rejected() {
    ...
    let forged = Known::<u64> {
        ...
        party: survivor.party.dangerously_alias(),
        tree: Tree {
            root: Root::default(),
        },
    };
...
/// [`Party::seed`] (`"1"`, the whole interval). Forking splits the seed's region
/// across the children; each `retire` hands a region back, and a leak anywhere
/// would leave the reunited party short of the whole.
#[test]
fn retiring_all_forks_reconstitutes_the_seed_party() {
    let mut survivor = Known::<u64>::seed();
    // Empty forks share the seed's (empty) version, so each is reflexively
    // dominated and retires with no prior gossip.
    let c1 = survivor.fork();
    let c2 = survivor.fork();
    let c3 = survivor.fork();
