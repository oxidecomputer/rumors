...
/// observed advance of the set's causal frontier, with no messages attached.
///
/// This is the wake-up signal for anything that reacts to "the set changed"
/// without consuming the changes themselves — above all the `when` input to
/// [`gossip_when`](crate::Rumors::gossip_when), but equally a
/// persist-on-change loop or a UI refresh. For the changes *themselves*, use
/// [`Messages`](crate::Messages) or
/// [`CausalMessages`](crate::CausalMessages).
///
/// # Ticks are a signal, not a ledger
///
/// The stream is *coalescing*: it yields one `()` for everything that
/// happened since the previous poll, however many commits that was, and it
/// yields immediately on first poll (a fresh observer has seen nothing, so
/// whatever the set holds is news). Consequently the number of ticks means
/// nothing; only "at least one tick since I last looked" does. Every commit
/// fires it — local [`send`](crate::Rumors::send)s and
/// [`redact`](crate::Rumors::redact)s, and joins learned by gossip on any
/// clone — because every commit advances the set's [`Version`]; that frontier,
/// not the observable content hash, is the change signal (two replicas can
/// hold byte-identical live content and still owe each other version
/// reconciliation).
///
/// The stream ends (`None`) once the [`Peer`](crate::Peer) and every
/// [`Rumors`](crate::Rumors) for the set have dropped and no further change
/// is possible. Like the message observers, holding a `Changes` does not
/// count against the quiescence that lets
...
/// connection deadlocks: each side's `gossip` leads with its preamble and
/// then waits for the peer's, so the side whose set did *not* change never
/// answers. A driver must also enter a session when the *remote* initiates —
/// which is exactly what [`gossip_when`](crate::Rumors::gossip_when) adds;
/// feed this stream to it rather than calling `gossip` yourself.
