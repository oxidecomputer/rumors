...
//!
//! A black box bounds three contract clauses; a pass leaves each partly the
//! implementation's own obligation:
//!
//! - **Bounded buffering.** The independence probe writes into a stalled
//!   stream the whole time the other streams take to finish, so it exposes
//!   cross-stream coupling hidden behind buffers of up to roughly that many
//!   bytes. An implementation that buffers more — in the limit, one that
//!   never backpressures at all — passes anyway.
//! - **Cancellation mid-delivery.** The probe can only drop an `accept`
//!   future mid-wait if the acceptor actually returns `Pending` at that
//!   moment. An acceptor that resolves on its first poll is never caught
//!   waiting, so whatever a drop at that moment would lose goes untested.
