...
deadlock-free at every depth pair and capacity. Message-denominated
...
C = 1 (`muxCompletesK`: completion in the literal kernel sense). The
K semantics, the arrears-K licensing, and both gate depths execute
for real. Message-denominated (Mux/Basic.lean, # The
byte-denomination caveat). -/
...
decision), accepted as for the K = 1 wedge pin (`muxCompletesK`:
completion in the literal kernel sense). Message-denominated
(Mux/Basic.lean, # The byte-denomination caveat). -/
