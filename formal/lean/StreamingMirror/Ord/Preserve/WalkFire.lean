/-
O preservation for `walkFire` — the hardest walk action, unchanged by
the dequeue-order class: firing is a base arm (`applyO`'s catch-all
delegates), and a fire step moves NO receive count, so the entire
per-obligation analysis (ledger re-establishment when staying, the
prefix-sum telescopes when advancing) transcribes verbatim from the
base file. The one delta is the flow assembler: both base receive
formulas at the fired walk are unchanged across the step, so the O
counts ride the `_congr` lemmas through `recvdOfO_setWalk_same` — no
per-assignment reasoning anywhere in this file. The base file's
per-obligation lemmas and their counting infrastructure are `private`,
so they re-land here as private twins (cited per docstring); the base
file's public telescopes (`wiresBefore_succ` and friends) and ledger
lemmas (`length_filter_insert`, `frontier_of_count`,
`foldl_add_update'`, `wkLocalOk_fresh`) are imported as-is.

Chain (ord, stage B): the walk fire-step preservation case, consumed
by Ord/Preserve.lean. Base mirror: Proofs/Preserve/WalkFire.lean.
Map: PROGRESS.md §13.
-/
...
-- Private twins of the base file's private counting layer, verbatim.

/-- A filter over `range fan` collapses to `range n` when the predicate
is dead above `n` (a private twin of the base `length_filter_range_ext`). -/
...
/-- Summing a constant-zero map is zero (a private twin of the base
`sum_map_zero`). -/
...
is dead above `n` (a private twin of the base `sum_range_ext`). -/
...
private twin of the base `length_filter_index`). -/
...
/-- A D child is a real child (a private twin of the base
...
/-- Only non-leaf stages have D children (a private twin of the base
`ne_zero_of_childIsD`). -/
...
the base `childIsD_eq_false_of_ge`). -/
...
/-- Query budgets exist only for D children (a private twin of the base
...
(a private twin of the base `wireCount_of_complete`). -/
...
private twin of the base `count_eq_dOf`). -/
...
private twin of the base `qSum_eq_qOf`). -/
...
`qOf` (a private twin of the base `qSum_of_complete`). -/
...
twin of the base `wkWireRecvd_fresh`). -/
...
twin of the base `wkAskedRecvd_fresh`). -/
...
/-- Producer counts never read channel occupancy (a private twin of the
base `sentOf_chan_irrel`). -/
private theorem sentOf_chan_irrel (ch : Chan → Nat) (c : Chan) :
    sentOf sk { s with chan := ch } c = sentOf sk s c := by
  cases c <;> rfl

/-- Sends frame for a walk-plus-channel update (a private twin of the
base `sentOf_fire_frame`; the producer side is order-blind). -/
...
/-- Assembles the flow field of `InvPO` for a fire step (a private twin
of the base `flow_fire_assemble`): the touched channel's occupancy
rises from empty to one exactly as its producer count rises by one;
every other channel frames — the O receive counts ride the two
unchanged base counts through `recvdOfO_setWalk_same`, at any
assignment. -/
private theorem flow_fireO_assemble (hwf : sk.wellFormed = true)
    ...
    (hi : InvPO sk ax ord s) :
    ∀ c ∈ allChans sk,
      s'.chan c + recvdOfO sk ord s' c = sentOf sk s' c
        ∧ s'.chan c ≤ sk.cap c := by
  intro c hc
...
/-- `walkFire` on a committed `.parent`, over the O invariant (a
private twin of the base `preserve_walkFire_parent`; the local analysis
is verbatim, the flow field rides `flow_fireO_assemble`). -/
private theorem preserve_walkFire_parentO (hwf : sk.wellFormed = true)
  ...
  · -- the scope completes: the walk advances to a fresh cursor
    ...
    refine ⟨fun pk' hpk' => ?_, fun pk' hpk' => ?_, ?_,
      flow_fireO_assemble hwf pk hmem' (upperOut pk) _ hs' hchan0 rfl
        ?_ ?_ ?_ ?_ ?_ ?_ ?_ hi⟩
...
/-- `walkFire` on a committed `.wire i`, over the O invariant (a
private twin of the base `preserve_walkFire_wire`). -/
private theorem preserve_walkFire_wireO (hwf : sk.wellFormed = true)
  ...
  · -- the scope completes: `i` was the last missing wire
    ...
    have hqc : (List.range sk.fan).foldl
      ...
      · exact h.2
    refine ⟨fun pk' hpk' => ?_, fun pk' hpk' => ?_, ?_,
      flow_fireO_assemble hwf pk hmem' (wireOut pk) _ hs' hchan0 rfl
        ?_ ?_ ?_ ?_ ?_ ?_ ?_ hi⟩
    · by_cases hpkeq : pk' = pk
      · subst hpkeq
        exact wkLocalOk_fresh pk' _ (by omega) hwalk'
...
/-- `walkFire` on a committed `.res i`, over the O invariant (a private
twin of the base `preserve_walkFire_res`). -/
private theorem preserve_walkFire_resO (hwf : sk.wellFormed = true)
    (pk : Party × Nat) (i : Nat) (hmem' : pk ∈ sk.walkKeys)
    (hph2 : (s.walk pk).phase = 2)
  ...
  · -- the scope completes: `i` was the last missing resolution
    ...
    have hqc : (List.range sk.fan).foldl
      ...
      · exact h.2
    refine ⟨fun pk' hpk' => ?_, fun pk' hpk' => ?_, ?_,
      flow_fireO_assemble hwf pk hmem' (lowerOut pk) _ hs' hchan0 rfl
        ?_ ?_ ?_ ?_ ?_ ?_ ?_ hi⟩
    · by_cases hpkeq : pk' = pk
      · subst hpkeq
        exact wkLocalOk_fresh pk' _ (by omega) hwalk'
...
/-- `walkFire` on a committed `.query i`, over the O invariant (a
private twin of the base `preserve_walkFire_query`). -/
private theorem preserve_walkFire_queryO (hwf : sk.wellFormed = true)
    (pk : Party × Nat) (i : Nat) (hmem' : pk ∈ sk.walkKeys)
    (hph2 : (s.walk pk).phase = 2)
  ...
  · -- the scope completes: this query saturated the last budget
    ...
    have hqof : (List.range sk.fan).foldl
      ...
      · intro j hj hd
        ...
        · exact h.2
    refine ⟨fun pk' hpk' => ?_, fun pk' hpk' => ?_, ?_,
      flow_fireO_assemble hwf pk hmem' (askedOut pk) _ hs' hchan0 hcap1
        ?_ ?_ ?_ ?_ ?_ ?_ ?_ hi⟩
    · by_cases hpkeq : pk' = pk
      · subst hpkeq
        exact wkLocalOk_fresh pk' _ (by omega) hwalk'
...
/-- `walkFire` under the assignment (a shared base arm): a committed
obligation publishes into its channel exactly as in the base model —
firing moves no receive count in either order, so the O invariant
rides the base per-obligation analysis unchanged (cf.
`preserve_walkFire`). -/
theorem preserve_walkFireO (hwf : sk.wellFormed = true)
    (pk : Party × Nat)
    (hstep : applyO sk ax ord (.walkFire pk) s = some s')
    (hi : InvPO sk ax ord s) : InvPO sk ax ord s' := by
  have hstep' : apply sk ax (.walkFire pk) s = some s' := hstep
  ...
  next o hcm =>
    ...
    case isTrue hg =>
      ...
      cases o with
      | wire i =>
          exact preserve_walkFire_wireO hwf pk i hmem' hph2 hcm hchan0 hs' hi
      | res i =>
          exact preserve_walkFire_resO hwf pk i hmem' hph2 hcm hchan0 hs' hi
      | query i =>
          exact preserve_walkFire_queryO hwf pk i hmem' hph2 hcm hchan0 hs' hi
      | parent =>
          exact preserve_walkFire_parentO hwf pk hmem' hph2 hcm hchan0 hs' hi
