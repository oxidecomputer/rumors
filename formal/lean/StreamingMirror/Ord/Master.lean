/-
The O master induction: `weaveGoE_wedge`'s twin over the
assignment-ordered interpreter, the pointwise readiness of the O
weave's every emission, and the closing wedge —

  `weaveO_wedge : wellFormed → margin 0 →
     WEdgeP sk (procsO sk ord) [] (weaveStateO sk ord)`

for EVERY assignment of the two-point dequeue-order class.

# The one structural delta

Each scope's two prologue receives dispatch on the scope's walk's
assignment (`prologueO`). Both receives discharge from their
in-flight predecessor sends through `enabled_rcv_of_memP`, which
never reads the future — under query-first the asked receive heads
the future and its enabledness prefix lacks the wire receive, which
is irrelevant (the asked channel's counts do not read the wire
channel), and symmetrically for the wire receive with the asked
receive already emitted. So the dispatch is ONE lemma
(`emitOK_prologueO`): the same two discharges, consed in the
branch's order. Everything after the prologue — the kid slots, the
tail parent, the telescopes — is prologue-free and transcribes from
MasterE.lean with the O names (`walkSegO` tails, `AncTeleO`
contexts, the `FamOKO` windows; at the leaf-wire site the tighter
`wire0_windowO` request-feed hypothesis is discharged from the site's
EXACT emitted-request count, which the E derivation already had and
merely weakened).

Chain (ord, stage D exit): edge respect; consumed by Ord/Final.
Base mirror: MasterE.lean. Map: PROGRESS.md §13.
-/
...
/-- THE O CONSUMPTION INDUCTION: `weaveGoE_wedge` over the
assignment-ordered expanders, at the O family. -/
theorem weaveGoO_wedge (fuel : Nat) :
    ∀ (ops : List WOp) (st : MState) (done : List Ev),
      WEdgeP sk (procsO sk ord) (goEventsO sk ord fuel ops) st →
      DepOK done (goEventsO sk ord fuel ops) →
      (∀ x ∈ done, x ∈ st.out) →
      EmitOKOnP sk (procsO sk ord) (goEventsO sk ord fuel ops) [] →
      step sk st = none →
      WEdgeP sk (procsO sk ord) [] (weaveGoO sk ord fuel ops st) := by
  induction fuel with
  ...
  | succ f ih =>
      ...
      | [] => exact hW
      | .emit e :: rest =>
          have hgo : goEventsO sk ord (f + 1) (.emit e :: rest)
              = e :: goEventsO sk ord f rest := rfl
          ...
          show WEdgeP sk (procsO sk ord) []
            (weaveGoO sk ord f rest (wEmitP sk st e))
...
/-- Pointwise emission-readiness at the O family. -/
abbrev EmitOKOnO (l rest : List Ev) : Prop :=
  EmitOKOnP sk (procsO sk ord) l rest

-- ==================================================== window plumbing

/-- A window conclusion opens the guard (family-free; private twin of
MasterE's `enabled_of_windowE`). -/
private theorem enabled_of_windowO {st : MState} {c : Chan} {n : Nat}
...
O family — position-free, so it serves either prologue order. -/
theorem head_rcv_wireO (hwf : sk.wellFormed = true) {fut : List Ev}
    {st : MState} (hW : WCountP sk (procsO sk ord) fut st) {p : Party}
    {hh n : Nat}
    (hpred : ∀ d, manDep ((Chan.wire p hh, false, n) : Ev) = some d →
      d ∈ st.out) :
    enabled sk st.sent st.rcvd (Chan.wire p hh, false, n) = true :=
  enabled_rcv_of_memP sk hW (procsO_snd_owned sk ord hwf)
    (procsO_canon sk ord _ true) (hpred _ (manDep_wire_rcv p hh n))

/-- A prologue asked receive discharges from its in-flight send, at
the O family — position-free, so it serves either prologue order. -/
theorem head_rcv_askedO (hwf : sk.wellFormed = true) {fut : List Ev}
    {st : MState} (hW : WCountP sk (procsO sk ord) fut st) {p : Party}
    {hh n : Nat}
  ...
  enabled_rcv_of_memP sk hW (procsO_snd_owned sk ord hwf)
    (procsO_canon sk ord _ true) (hpred _ (manDep_asked_rcv p hh n))

/-- A manual-consumed wire send discharges from its predecessor
receive, or opens on a fresh window at seq zero (O family). -/
theorem head_snd_wireO (hwf : sk.wellFormed = true) {fut : List Ev}
    {st : MState} (hW : WCountP sk (procsO sk ord) fut st) {p : Party}
    {hh n : Nat} (hh1 : 1 ≤ hh)
  ...
  · have hc : sk.cap (Chan.wire p hh) = 1 := rfl
    refine enabled_snd_of_memP sk hW (procsO_rcv_owned sk ord hwf)
      (procsO_canon sk ord _ false) ?_ (by omega)
...
receive, or opens on a fresh window at seq zero (O family). -/
theorem head_snd_askedO (hwf : sk.wellFormed = true) {fut : List Ev}
    {st : MState} (hW : WCountP sk (procsO sk ord) fut st) {p : Party}
    {hh n : Nat}
...
-- ====================================== THE prologue-order dispatch

/-- Readiness of an ordered prologue against ANY tail: each of the two
receives discharges from its in-flight predecessor send
(`enabled_rcv_of_memP` never reads the future), so both orders of the
two-point class discharge with the SAME two head lemmas, consed in
the assignment's order. The O master induction's one structural delta
versus MasterE lives entirely in this dispatch. -/
theorem emitOK_prologueO (hwf : sk.wellFormed = true) (hh k : Nat)
    (rest : List Ev) :
    EmitOKOnP sk (procsO sk ord) (prologueO ord (wpk hh) k) rest := by
  cases hord : ord.walk (wpk hh) with
  | replyFirst =>
      rw [show prologueO ord (wpk hh) k
          = [((wireIn (wpk hh), false, k) : Ev),
             (askedIn (wpk hh), false, k)] from by
        unfold prologueO; rw [hord]]
      refine emitOKOn_cons sk (fun st hW hfix hpred =>
        head_rcv_wireO sk ord hwf hW.toWCountP hpred) ?_
      exact emitOKOn_cons sk (fun st hW hfix hpred =>
        head_rcv_askedO sk ord hwf hW.toWCountP hpred)
        (emitOKOn_nil sk _)
  | queryFirst =>
      rw [show prologueO ord (wpk hh) k
          = [((askedIn (wpk hh), false, k) : Ev),
             (wireIn (wpk hh), false, k)] from by
        unfold prologueO; rw [hord]]
      refine emitOKOn_cons sk (fun st hW hfix hpred =>
        head_rcv_askedO sk ord hwf hW.toWCountP hpred) ?_
      exact emitOKOn_cons sk (fun st hW hfix hpred =>
        head_rcv_wireO sk ord hwf hW.toWCountP hpred)
        (emitOKOn_nil sk _)

-- ======================================== the O kid-suffix alignment

private theorem drop_eq_flatMap_getElem?O {α : Type _} :
  ...
  | m + 1, i, F, h => by
      rw [List.range'_succ, List.flatMap_cons,
        drop_eq_flatMap_getElem?O m (i + 1) F (by omega),
        List.getElem?_eq_getElem (by omega), Option.toList_some,
        List.singleton_append, ← List.drop_eq_getElem_cons (by omega)]

private theorem walkSegO_glue_range' (h' : Nat) (g : Nat → Nat)
    (hmono : ∀ i, g i ≤ g (i + 1)) :
    ∀ (m i : Nat),
      (List.range' i m).flatMap
          (fun j => walkSegO sk ord h' (g j) (g (j + 1)))
        = walkSegO sk ord h' (g i) (g (i + m)) := by
  ...
  | succ m ihm =>
      ...
      rw [List.range'_succ, List.flatMap_cons, ihm (i + 1),
        walkSegO_glue sk ord (hmono i) (hchain m (i + 1)),
        show i + 1 + m = i + (m + 1) from by omega]

/-- One O kid op's per-stage filters (cf. `kid_filtersE`): the
ownership cover, the feeder's query, the own-stage chunk — a plain
`childChunk`, never a parent, never a prologue — and the descendant O
windows. -/
theorem kid_filtersO (hwf : sk.wellFormed = true)
    ...
    (∀ e ∈ opEventsO sk ord (.kid h k (sk.stageScope h k) none
        (sk.wiresBefore h k) i F),
      evOwner sk e = mF ∨ ∃ h', h' ≤ h ∧ evOwner sk e = walkIdx sk h')
    ∧ (opEventsO sk ord (.kid h k (sk.stageScope h k) none
  ...
  | zero =>
      ...
      have hkidE : opEventsO sk ord (.kid 0 k (sk.stageScope 0 k)
  ...
  | succ h =>
      ...
      have hkidE : opEventsO sk ord (.kid (h + 1) k
            (sk.stageScope (h + 1) k)
        ...
        simp only [Nat.add_sub_cancel,
          ...
          if_false]
      have hIHsub := align_scopeO sk ord hwf h
      ...
      have hIHW := fun
        ...
        align_scopeO sk ord hwf h (sk.wiresBefore (h + 1) k + i) []
          (walkIdx sk (h + 1)) hh' hkid
          (by
      ...
      · -- ownership
        ...
        · rw [if_neg hD] at he
          ...
          · rcases (hIHW hDf).1 e he with ho | ⟨h'', hle'', ho⟩
            ...
            · exact Or.inr ⟨h'', by omega, ho⟩
      · -- the feeder's filter
        have hSubDropD : (opEventsO sk ord (.scope h
              (sk.wiresBefore (h + 1) k + i)
        ...
        · have hDf : sk.childIsD (h + 1) (sk.stageScope (h + 1) k) i
              = false := by simpa using hD
          have hSubDropW : (opEventsO sk ord (.scope h
...
/-- The O kid-suffix clauses (cf. `align_kids_suffixE`): ownership,
the feeder's residue, the own-stage chunk run — parent-free,
prologue-free — and the descendant O windows. -/
theorem align_kids_suffixO (hwf : sk.wellFormed = true)
    {h k : Nat} (hh : h < sk.rootH) (hk : k < sk.stageLen h)
    {F : List Ev} {mF : Nat}
    (hF : F.length = sk.nChildren h (sk.stageScope h k))
    (hFo : ∀ e ∈ F, evOwner sk e = mF)
    ...
    (∀ e ∈ (List.range' i (sk.nChildren h (sk.stageScope h k) - i)).flatMap
        (fun j => opEventsO sk ord (.kid h k (sk.stageScope h k)
  ...
  · intro e he
    ...
    exact (kid_filtersO sk ord hwf hh hk hF hFo hmF (hjlt j hj)).1 e hej
  · simp only [List.filter_flatMap]
    rw [flatMap_congr (fun j hj =>
      (kid_filtersO sk ord hwf hh hk hF hFo hmF (hjlt j hj)).2.1)]
    exact drop_eq_flatMap_getElem?O
      (sk.nChildren h (sk.stageScope h k) - i) i F (by omega)
  · simp only [List.filter_flatMap]
    exact flatMap_congr (fun j hj =>
      (kid_filtersO sk ord hwf hh hk hF hFo hmF (hjlt j hj)).2.2.1)
  · intro h' hlt
    simp only [List.filter_flatMap]
    rw [flatMap_congr (fun j hj =>
        (kid_filtersO sk ord hwf hh hk hF hFo hmF (hjlt j hj)).2.2.2
          h' hlt),
...
/-- An O subtree owns nothing at a foreign index. -/
theorem scope_filter_neO (hwf : sk.wellFormed = true) {h k : Nat}
  ...
  rw [List.filter_eq_nil_iff]
  intro e he
  rcases (align_scopeO sk ord hwf h k F mF hh hk hF hFo hmF).1 e he with
    ho | ⟨h', hle, ho⟩
  · simp only [ho, beq_iff_eq]
    exact hMne
  · simp only [ho, beq_iff_eq]
...
/-- An O kid suffix owns nothing at a foreign index. -/
theorem kids_filter_neO (hwf : sk.wellFormed = true) {h k : Nat}
    {F : List Ev} {mF M : Nat} (hh : h < sk.rootH)
    (hk : k < sk.stageLen h)
    (hF : F.length = sk.nChildren h (sk.stageScope h k))
    (hFo : ∀ e ∈ F, evOwner sk e = mF) (hmF : mF < walkIdx sk h)
  ...
  rw [List.filter_eq_nil_iff]
  intro e he
  rcases (align_kids_suffixO sk ord hwf hh hk hF hFo hmF hi).1 e he with
    ho | ⟨h', hle, ho⟩
  · simp only [ho, beq_iff_eq]
    exact hMne
  · simp only [ho, beq_iff_eq]
...
-- ================================= rebasing and gluing the O context

/-- Rebase the O telescope across a local prefix (cf.
`ancTeleE_rebase`). -/
theorem ancTeleO_rebase {h : Nat} {A j t : Nat → Nat}
    {pre rest : List Ev} (hanc : AncTeleO sk ord h A j t rest)
      ...
      (pre ++ rest).filter
        ...
        = (chunkQ sk (h + 1) (A (h + 1)) (j (h + 1))).drop c
          ...
          ++ ((upperOut (wpk (h + 1)), true, A (h + 1)) : Ev)
            :: walkSegO sk ord (h + 1) (A (h + 1) + 1)
                (sk.stageLen (h + 1))) :
    AncTeleO sk ord h A j (fun G => if G = h + 1 then c else t G)
      (pre ++ rest) := by
...
/-- The deep O windows at a mid-scope slot: the kid suffix's windows
glued to the after-scope remainder (cf. `deep_glueE`). -/
theorem deep_glueO (hwf : sk.wellFormed = true) {h k : Nat}
    ...
    (hrest : ∀ g', g' < h →
      rest.filter (fun e => evOwner sk e == walkIdx sk g')
        = walkSegO sk ord g' (descIdx sk g' (h - g') (k + 1))
            (sk.stageLen g')) :
    ∀ g', g' < h →
  ...
  have hend : descIdx sk g' (h - 1 - g')
    ...
    · rw [show g' + (h - g') = h from by omega]
      exact hk
  exact walkSegO_glue sk ord hmono hend

-- ==================================== the O site futLen floors

private theorem qSum_monoO (pk : Party × Nat) (k : Nat) :
...
/-- The O resolution site's floors: the remaining resolution share
with its bound, and the pending-parent summary share — no splice
discriminant (cf. `futLenE_site_lower`). -/
private theorem futLenO_site_lower {fut : List Ev} {h k i : Nat}
    (hk : k < sk.stageLen h)
    (hi : i < sk.nChildren h (sk.stageScope h k))
  ...
  · have hqne : proj (upperOut (wpk h)) true (chunkQ sk h k i) = [] :=
    ...
    rw [futLen_of_filter sk hfil,
      ...
      chunksNone_proj_upper, proj_cons_self, walkSegO_proj_eq,
      walkSeg_proj_upper sk (show k + 1 ≤ sk.stageLen h by omega)]
    simp only [List.nil_append, List.length_cons, seg_len]
    omega

/-- The O resolution site's query floor (cf. `futLenE_SL_q`). -/
private theorem futLenO_SL_q {fut : List Ev} {h k i : Nat}
    (hk : k < sk.stageLen h)
    (hi : i < sk.nChildren h (sk.stageScope h k))
    (hfil : fut.filter (fun e => evOwner sk e == walkIdx sk h)
      = ((lowerOut (wpk h), true,
  ...
  have hqm : qSum sk (wpk h) k (i + 1)
    ...
    qSum_monoO sk (wpk h) k hi
  omega

/-- The O wire site's floor and bound (cf. `futLenE_site_wire`): the
pending parent rides the tail and contributes nothing. -/
private theorem futLenO_site_wire {fut : List Ev} {h k i : Nat}
    (hk : k < sk.stageLen h)
    (hi : i < sk.nChildren h (sk.stageScope h k))
    (hfil : fut.filter (fun e => evOwner sk e == walkIdx sk h)
      = (List.range' i
...
/-- The O leaf-request site's wire count (cf. `futLenE_Q0_wire`). -/
private theorem futLenO_Q0_wire {fut : List Ev} {k i0 : Nat}
    (hk : k < sk.stageLen 0)
    ...
    (hfil : fut.filter (fun e => evOwner sk e == walkIdx sk 0)
      = (List.range' (i0 + 1)
          ...
          ++ ((upperOut (wpk 0), true, k) : Ev)
            :: walkSegO sk ord 0 (k + 1) (sk.stageLen 0)) :
...
/-- The O ancestor query floor at a feed cursor (cf. `futLenE_site_q`),
read off the telescope's fil shape. -/
private theorem futLenO_site_q {fut : List Ev} {h K i t : Nat}
    (hK : K < sk.stageLen h)
    (hi : i < sk.nChildren h (sk.stageScope h K))
...
/-- The O parent site (cf. `ready_upperE`): a scope's tail summary is
emittable through the O upper window — descent from the clean subtree
boundary, ascent and `P1` from margin 0. -/
theorem ready_upperO (hwf : sk.wellFormed = true)
    (hm0 : ∀ s, sk.dCount s ≤ sk.capLevel) {fut : List Ev} {h k : Nat}
    {A j t : Nat → Nat} (hhr : h < sk.rootH) (hk : k < sk.stageLen h)
    (hanc : AncTeleO sk ord h A j t fut)
    (hcoh0 : h + 1 < sk.rootH →
    ...
    (hown : fut.filter (fun e => evOwner sk e == walkIdx sk h)
      = ((upperOut (wpk h), true, k) : Ev)
          :: walkSegO sk ord h (k + 1) (sk.stageLen h)) :
    ∀ st : MState, WEdgeP sk (procsO sk ord) fut st →
      step sk st = none →
  ...
  have hasks : asks ((wpk h).1) (h + 1) = true := by
    ...
    rw [hna] at hs
    simpa using hs
  have hfu := futLen_siteO_upper sk ord hk hown
  have hsnd : sndCount (Chan.upper ((wpk h).1) h) st.out = k :=
    upper_site_hsndO sk ord hwf hW.toWCountP hna hhr hk hfu
  have hcov := ancTele_covO sk ord hwf hm0 hW hanc hcoh0 hsnd
  have hroot := root_bankedO sk ord hwf hW.toWCountP hfeed
  have hdesc : DescSupply sk st ((wpk h).1) h
    ...
    · have hfl := futLen_siteO_upper_res sk ord hk hown
      have hlpin := lower_snd_pinO sk ord hwf hW.toWCountP hhr
      ...
      have hXle : sk.wiresBefore h (k + 1) ≤ sk.stageLen (h - 1) := by
        ...
        omega
      refine descSupply_upper_of_ctxO sk ord hwf hW.toWCountP h1 hhr hk
        hasks hXW hXle hdeep (by omega) ?_
      intro h1'
      subst h1'
      have hfq := futLen_siteO_upper_q sk ord hk hown
      have hqpin := asked_snd_pinO sk ord hwf hW.toWCountP
        (Nat.le_refl 1) hhr
      ...
      omega
  have hwin := upper_windowO sk ord hwf (famOKO_procsO sk ord hwf) hW
    hfix (wpk_htop sk h) hasks (wtop_ge hwf hhr) hk hsnd hdesc hcov
    hroot
  exact enabled_of_windowO sk hwf hwin (hW.rcvd_eq _)

/-- The O resolution site (cf. `ready_lowerE`): margin 0 supplies the
schedulable slack and the ascent overhangs. -/
theorem ready_lowerO (hwf : sk.wellFormed = true)
  ...
  have hasks : asks ((wpk h).1) (h + 1) = true := by
    ...
    rw [hna] at hs
    simpa using hs
  obtain ⟨hfl, hbnd, hfu⟩ := futLenO_site_lower sk ord hk hi hD hown
  have hsnd := lower_site_hsndO sk ord hwf hW.toWCountP hna hhr hfl hbnd
  have hupk := upper_site_hsndO sk ord hwf hW.toWCountP hna hhr hk hfu
  ...
  have hcov : AscCover sk st ((wpk h).1) (h + 1) (wtop sk h) :=
    ascCover_pred sk (ancTele_covO sk ord hwf hm0 hW hanc hcoh0 hupk)
      hasks
  have hq1 : h = 1 →
    ...
    intro h1'
    subst h1'
    have hfq := futLenO_SL_q sk ord hk hi hown
    have hqpin := asked_snd_pinO sk ord hwf hW.toWCountP (Nat.le_refl 1)
      hhr
    ...
    have hqs1 : qSum sk (wpk 1) k i
      ...
      qSum_monoO sk (wpk 1) k (Nat.le_of_lt hi)
    ...
    rw [show Chan.leafRequests = askedOut (wpk 1) from rfl]
    omega
  have hdesc := descSupply_lower_of_ctxO sk ord hwf hW.toWCountP h1 hhr
    hk (Nat.le_of_lt hi) hna hdeep hq1
  have hd : sk.dsBefore h k + dRank sk (wpk h) k i
      < (sk.asmResList ((wpk h).1) h).length := by
    rw [answerer_resList_total hwf hna h1 hhr]
    exact hbnd
  rw [hsnd] at hp1full
  have hwin := lower_windowO sk ord hwf (famOKO_procsO sk ord hwf) hW
    hfix (wpk_htop sk h) hna h1
    (show h < wtop sk h from by have := wtop_ge hwf hhr; omega)
    hd hsnd hp1full hdesc hcov hroot
  exact enabled_of_windowO sk hwf hwin (hW.rcvd_eq _)

/-- The O leaf-wire site (cf. `ready_wire0E`). The tighter
`wire0_windowO` request-feed hypothesis discharges from the site's
EXACT emitted-request count — the stage-0 walk emits slot `j`'s wire
and THEN slot `j`'s feed request, so at the slot-`i0` wire site the
requests of slots `0..i0−1` are all emitted and the count equality
covers any slack `1 − wirePhase ≥ 0` outright. -/
theorem ready_wire0O (hwf : sk.wellFormed = true)
    (hm0 : ∀ s, sk.dCount s ≤ sk.capLevel) {fut : List Ev} {k i0 : Nat}
    {A j t : Nat → Nat} (hr : 0 < sk.rootH) (hk : k < sk.stageLen 0)
    (hi0 : i0 < sk.nChildren 0 (sk.stageScope 0 k))
    (hanc : AncTeleO sk ord 0 A j t fut)
  ...
  obtain ⟨hfw, hwbnd⟩ := futLenO_site_wire sk ord hk hi0 hown
  have hsnd := wire0_site_hsndO sk ord hwf hW.toWCountP hr hfw hwbnd
  ...
  have hqc : sk.qCount 1 (sk.stageScope 1 (A 1)) (j 1)
    ...
    exact hq
  obtain ⟨hfq, hqbnd⟩ := futLenO_site_q sk ord hA1 hj1
    (by rw [ht1, hqc]; exact hi0) (hanc.fil 1 (by omega) hr2)
  have hsndq := leafreq_site_hsndO sk ord hwf hW.toWCountP hr2 hfq hqbnd
  have hqw := qs_wires_mid sk hwf (Nat.le_refl 1) hr2 hA1
    (Nat.le_of_lt hj1)
  rw [show (1 : Nat) - 1 = 0 from rfl] at hqw
  rw [hqw, ← hcoh0, ht1] at hsndq
  -- the exact count subsumes the assignment-tight slack: at
  -- query-first the slack is zero, and the feed count IS the wire seq
  have hreq : sk.wiresBefore 0 k + i0
      ≤ sndCount Chan.leafRequests st.out
        + (1 - ord.absorb.wirePhase) := by omega
  have hcov := ancTele_cov_leafO sk ord hwf hm0 hW hanc hr2 hk hcoh0
    hi0 hsndq
  have hroot := root_bankedO sk ord hwf hW.toWCountP hfeed
  have hwin := wire0_windowO sk ord hwf (famOKO_procsO sk ord hwf) hW
    hfix hw hsnd hreq hcov hroot
  exact enabled_of_windowO sk hwf hwin (hW.rcvd_eq _)

/-- The O leaf-request site (cf. `ready_leafreqE`). -/
theorem ready_leafreqO (hwf : sk.wellFormed = true)
    (hm0 : ∀ s, sk.dCount s ≤ sk.capLevel) {fut : List Ev} {k i0 : Nat}
    {A j t : Nat → Nat} (hr : 0 < sk.rootH) (hk : k < sk.stageLen 0)
    (hi0 : i0 < sk.nChildren 0 (sk.stageScope 0 k))
    (hanc : AncTeleO sk ord 0 A j t fut)
  ...
  have hq : sk.wiresBefore 0 k + i0 < sk.totalLeafReqs := by
    ...
    have hcong : sk.wiresBefore 0 k
      ...
      rw [hcoh0]
    omega
  have hfw := futLenO_Q0_wire sk ord hk hi0 hown
  have hwpin := wire_snd_pinO sk ord hwf hW.toWCountP hr
  ...
  have hcov := ancTele_cov_leafO sk ord hwf hm0 hW hanc hr2 hk hcoh0
    hi0 hsndq
  have hroot := root_bankedO sk ord hwf hW.toWCountP hfeed
  have hwin := leafreq_windowO sk ord hwf (famOKO_procsO sk ord hwf) hW
    hfix hq hsndq hwire hcov hroot
  exact enabled_of_windowO sk hwf hwin (hW.rcvd_eq _)

-- ============================================== the leaf-stage scopes

/-- The O master induction's leaf case: the ordered prologue receives
from their in-flight predecessors, each slot's wire and feed query
through the absorber windows, and the parent summary LAST through the
upper window — the epilogue placement (cf. `emitOK_scope_zeroE`). -/
theorem emitOK_scope_zeroO (hwf : sk.wellFormed = true)
    (hm0 : ∀ s, sk.dCount s ≤ sk.capLevel) {k : Nat} {rest : List Ev}
    {A j t : Nat → Nat} (hk : k < sk.stageLen 0)
    (hlow : ∀ g', g' ≤ 0 →
      rest.filter (fun e => evOwner sk e == walkIdx sk g')
        = walkSegO sk ord g' (descIdx sk g' (0 - g') (k + 1))
            (sk.stageLen g'))
    (hanc : AncTeleO sk ord 0 A j t rest)
    (hcoh0 : k = sk.wiresBefore 1 (A 1) + j 1)
    (hsat : (chunkQ sk 1 (A 1) (j 1)).drop (t 1) = [])
    (hfd : ∃ i₀, rest.filter (fun e => evOwner sk e == 1)
      = ((ropenEvents sk).drop 3).drop i₀) :
    EmitOKOnO sk ord (opEventsO sk ord (.scope 0 k (scopeFeed sk 0 k)))
      rest := by
  have hr2 : 1 < sk.rootH := by have := (wf_rootH hwf).2; omega
  ...
  have hlow0 : rest.filter (fun e => evOwner sk e == walkIdx sk 0)
      = walkSegO sk ord 0 (k + 1) (sk.stageLen 0) := by
  ...
  have hfil1 : rest.filter (fun e => evOwner sk e == walkIdx sk 1)
      = (List.range' (j 1 + 1)
        ...
        ++ ((upperOut (wpk 1), true, A 1) : Ev)
          :: walkSegO sk ord 1 (A 1 + 1) (sk.stageLen 1) := by
  ...
  have hancU : AncTeleO sk ord 0 A j t
    ...
    exact hanc.fil G hG hGr
  -- the expansion
  have hE := opEventsO_scope_eq sk ord (Nat.le_of_lt hk)
    (scopeFeed sk 0 k)
  rw [List.range_eq_range'] at hE
  rw [hE]
  refine emitOKOn_append sk (emitOK_prologueO sk ord hwf 0 k _) ?_
  refine emitOKOn_append sk ?_ ?_
  · -- the leaf slots, folded against the pending parent
    have hfold : ∀ (m i : Nat),
        i + m = sk.nChildren 0 (sk.stageScope 0 k) →
        EmitOKOnP sk (procsO sk ord)
          ((List.range' i m).flatMap fun i' =>
            opEventsO sk ord (.kid 0 k (sk.stageScope 0 k)
      ...
      | succ m ihm =>
          ...
          have hi : i < sk.nChildren 0 (sk.stageScope 0 k) := by
            omega
          obtain ⟨-, hksI2, hksI3, -⟩ :=
            align_kids_suffixO sk ord hwf hr0 hk hF hFo hmF
              (i := i + 1) (by omega)
          have hkidE : opEventsO sk ord (.kid 0 k (sk.stageScope 0 k)
                none (sk.wiresBefore 0 k) i (scopeFeed sk 0 k))
              = ((wireOut (wpk 0), true,
          ...
          have hkn1 : ((List.range' (i + 1)
              ...
              (fun i' => opEventsO sk ord (.kid 0 k
                ...
                (scopeFeed sk 0 k)))).filter
              (fun e => evOwner sk e == 1) = [] :=
            kids_filter_neO sk ord hwf hr0 hk hF hFo hmF
              (i := i + 1) (by omega) (M := 1)
          ...
          · -- W0: the slot's wire through the absorber's wire window
            ...
            have hpreW : (((wireOut (wpk 0), true,
              ...
              rw [List.filter_cons_of_neg (by
                ...
                ← hdropi]
            refine ready_wire0O sk ord hwf hm0 hr0 hk hi (A := A)
              (j := j) (t := fun G => if G = 0 + 1 then i else t G)
              ?_ hcoh0 rfl ?_ ?_ st hW hfix
            · refine ancTeleO_rebase sk ord
            ...
            have hpreQ : (((askedOut (wpk 1), true,
              ...
              rw [List.filter_cons_of_pos (by
                  simp only [evOwner_askedOut sk (Nat.le_refl 1) hr2,
                    beq_self_eq_true]),
                hksI2, ← hdropi]
            refine ready_leafreqO sk ord hwf hm0 hr0 hk hi (A := A)
              (j := j) (t := fun G => if G = 0 + 1 then i else t G)
              ?_ hcoh0 rfl ?_ ?_ st hW hfix
            · refine ancTeleO_rebase sk ord
  ...
  · -- the tail parent through the upper window
    ...
    intro st hW hfix _
    rw [List.nil_append] at hW
    refine ready_upperO sk ord hwf hm0 hr0 hk hancU (fun _ => hcoh0) ?_
...
/-- The O kids-fold at an interior stage (cf. `emitOK_kidsE`): wires
...
hypothesis with the pushed rolling context. The parent summary rides
`rest`'s head, placed by the scope's own expansion; kid ops carry no
prologue, so this fold is order-blind. -/
private theorem emitOK_kidsO (hwf : sk.wellFormed = true)
    ...
    (hlowD : ∀ g', g' ≤ hp →
      rest.filter (fun e => evOwner sk e == walkIdx sk g')
        = walkSegO sk ord g' (descIdx sk g' (hp + 1 - g') (k + 1))
            (sk.stageLen g'))
    (hlowO : rest.filter (fun e => evOwner sk e == walkIdx sk (hp + 1))
      = ((upperOut (wpk (hp + 1)), true, k) : Ev)
          :: walkSegO sk ord (hp + 1) (k + 1) (sk.stageLen (hp + 1)))
    (hanc : AncTeleO sk ord (hp + 1) A j t rest)
    ...
    (IH : ∀ (k' : Nat) (rest' : List Ev) (A' j' t' : Nat → Nat),
      ...
      (∀ g', g' ≤ hp →
        rest'.filter (fun e => evOwner sk e == walkIdx sk g')
          = walkSegO sk ord g' (descIdx sk g' (hp - g') (k' + 1))
              (sk.stageLen g')) →
      AncTeleO sk ord hp A' j' t' rest' →
      (hp + 1 < sk.rootH →
      ...
      (∀ (pre : List Ev) (c : Nat),
        ...
        ∃ i₀, (pre ++ rest').filter (fun e => evOwner sk e == 1)
          = ((ropenEvents sk).drop 3).drop i₀) →
      EmitOKOnO sk ord
        (opEventsO sk ord (.scope hp k' (scopeFeed sk hp k')))
        rest') :
    ∀ (m i : Nat),
      i + m = sk.nChildren (hp + 1) (sk.stageScope (hp + 1) k) →
      EmitOKOnP sk (procsO sk ord)
        ((List.range' i m).flatMap fun i' =>
          opEventsO sk ord (.kid (hp + 1) k (sk.stageScope (hp + 1) k)
  ...
  | succ m ihm =>
      ...
      have hmF' : walkIdx sk (hp + 1) < walkIdx sk hp :=
        walkIdx_lt sk (by omega) hh
      obtain ⟨-, hsc2, hsc3⟩ := align_scopeO sk ord hwf hp
        (sk.wiresBefore (hp + 1) k + i)
        (scopeFeed sk hp (sk.wiresBefore (hp + 1) k + i))
        (walkIdx sk (hp + 1)) (by omega) hkid hF' hFo' hmF'
      have hscnil : ∀ M, walkIdx sk (hp + 1) ≠ M →
          (∀ h', h' ≤ hp → walkIdx sk h' ≠ M) →
          (opEventsO sk ord (.scope hp (sk.wiresBefore (hp + 1) k + i)
              (scopeFeed sk hp
                (sk.wiresBefore (hp + 1) k + i)))).filter
            (fun e => evOwner sk e == M) = [] :=
        fun M h1 h2 => scope_filter_neO sk ord hwf (by omega) hkid hF'
          hFo' hmF' h1 h2
      ...
      obtain ⟨-, hks2, hks3, hks4⟩ := align_kids_suffixO sk ord hwf hh
        hk (scopeFeed_length sk (hp + 1) k) hFo hmF (i := i + 1)
        (by omega)
      have hksU : ∀ M, (∀ h', h' ≤ hp + 1 → walkIdx sk h' ≠ M) →
          ...
          ((List.range' (i + 1)
              (sk.nChildren (hp + 1) (sk.stageScope (hp + 1) k)
                - (i + 1))).flatMap
            (fun i' => opEventsO sk ord (.kid (hp + 1) k
              (sk.stageScope (hp + 1) k) none
              (sk.wiresBefore (hp + 1) k) i'
              (scopeFeed sk (hp + 1) k)))).filter
            (fun e => evOwner sk e == M) = [] :=
        fun M h1 h2 => kids_filter_neO sk ord hwf hh hk
      ...
      have hglue := deep_glueO sk ord hwf (h := hp + 1) hh hk
        (i := i + 1)
        (by omega) hks4 (fun g' hg' => hlowD g' (by omega))
      -- the tail of the fold, converted
      have hih : EmitOKOnP sk (procsO sk ord) L rest := by
          ...
          (L ++ rest).filter
            ...
            = (chunkQ sk (hp + 1 + 1) (A (hp + 1 + 1))
              ...
              ++ ((upperOut (wpk (hp + 1 + 1)), true,
                    A (hp + 1 + 1)) : Ev)
                :: walkSegO sk ord (hp + 1 + 1) (A (hp + 1 + 1) + 1)
      ...
      have htele_lat : AncTeleO sk ord (hp + 1) A j
          (fun G => if G = hp + 1 + 1 then i + 1 else t G)
          (L ++ rest) :=
        ancTeleO_rebase sk ord hanc hnilU hparlat
      ...
      have htele_sub : AncTeleO sk ord hp
      ...
      have hfdsub : ∀ (pre : List Ev) (c : Nat),
        ...
        exact hlr
      -- the subtree, ready
      have hsubOK : EmitOKOnP sk (procsO sk ord)
          (opEventsO sk ord (.scope hp (sk.wiresBefore (hp + 1) k + i)
            (scopeFeed sk hp (sk.wiresBefore (hp + 1) k + i))))
          (L ++ rest) := by
      ...
      have hkidE := opEventsO_kid_eq sk ord (hp + 1) k none
      ...
      · -- a disputed slot: wire, resolution, query, subtree
        ...
        refine emitOKOn_cons sk (fun st hW hfix hpred =>
          head_snd_wireO sk ord hwf hW.toWCountP
            (show 1 ≤ hp + 1 by omega) hpred) ?_
        refine emitOKOn_cons sk ?_ ?_
        · intro st hW hfix _
          rw [List.cons_append] at hW
          refine ready_lowerO sk ord hwf hm0
        ...
        · refine emitOKOn_cons sk ?_ ?_
          · intro st hW hfix hpred
            have haq : askedOut (wpk (hp + 1 + 1))
              ...
              rfl
            rw [haq] at hpred ⊢
            exact head_snd_askedO sk ord hwf hW.toWCountP hpred
      ...
      · -- an undisputed slot: wire, query, childless subtree
        ...
        rw [show opEventsO sk ord (.scope hp
              (sk.wiresBefore (hp + 1) k + i) [])
            = opEventsO sk ord (.scope hp
                (sk.wiresBefore (hp + 1) k + i)
                (scopeFeed sk hp (sk.wiresBefore (hp + 1) k + i)))
...
/-- THE O MASTER INDUCTION: every emission of an assignment-ordered
scope's subtree is ready, given the scope's rolling entry context. The
site sequence per scope is the ordered prologue, per-kid chunks, then
ONE tail parent site; the capacity hypothesis (margin 0) replaces
`schedulable` throughout (cf. `emitOK_scopeE`). -/
theorem emitOK_scopeO (hwf : sk.wellFormed = true)
    ...
    ∀ (k : Nat) (rest : List Ev) (A j t : Nat → Nat) (mF : Nat),
      ...
      (∀ (pre : List Ev) (c : Nat),
        ...
        ∃ i₀, (pre ++ rest).filter (fun e => evOwner sk e == 1)
          = ((ropenEvents sk).drop 3).drop i₀) →
      EmitOKOnO sk ord (opEventsO sk ord (.scope h k (scopeFeed sk h k)))
        rest := by
  intro h
  induction h with
  | zero =>
      intro hh k rest A j t mF hk hFo hmF hmFeq hlow hanc hcoh0 hsat
        hfd
      have hr2 : 1 < sk.rootH := by have := (wf_rootH hwf).2; omega
      refine emitOK_scope_zeroO sk ord hwf hm0 hk hlow hanc (hcoh0 hr2)
        (hsat hr2) ?_
  ...
  | succ hp ih =>
      ...
      rw [List.range_eq_range'] at hE
      rw [hE]
      refine emitOKOn_append sk
        (emitOK_prologueO sk ord hwf (hp + 1) k _) ?_
      have hlow1 : rest.filter
            (fun e => evOwner sk e == walkIdx sk (hp + 1))
          = walkSegO sk ord (hp + 1) (k + 1)
              (sk.stageLen (hp + 1)) := by
      ...
      have hlowD' : ∀ g', g' ≤ hp →
        ...
        rw [List.filter_cons_of_neg (by
          ...
          hlow g' (by omega)]
      have hancU : AncTeleO sk ord (hp + 1) A j t
      ...
      · -- the kid slots against the pending parent
        refine emitOK_kidsO sk ord hwf hm0 hh hk hFo hmF hmFeq hlowD'
      ...
      · -- the tail parent through the upper window
        ...
        have hdeepU : ∀ g', g' < hp + 1 →
          ...
          rw [show hp + 1 - g' = (hp + 1 - 1 - g') + 1 from by omega,
            ...
            at hd
          exact hd
        refine ready_upperO sk ord hwf hm0 hh hk hancU hcoh0 ?_ hdeepU
          hlowO st hW hfix
...
/-- The full opening O future is pointwise-ready: the openers by
their seq-zero windows and in-flight predecessors, the root scope by
the O master induction with the trivial entry context (cf.
`emitOK_weaveE`). -/
theorem emitOK_weaveO (hwf : sk.wellFormed = true)
    (hm0 : ∀ s, sk.dCount s ≤ sk.capLevel) :
    EmitOKOnO sk ord ((weaveOps sk).flatMap (opEventsO sk ord)) [] := by
  have hge := (wf_rootH hwf).2
  rw [weave_flatMapO]
  refine emitOKOn_append sk ?_ ?_
  · -- the five openers
    show EmitOKOnP sk (procsO sk ord)
  ...
  · -- the root scope, entered with the trivial context
    ...
    refine emitOK_scopeO sk ord hwf hm0 (sk.rootH - 1) (by omega) 0 []
    ...
    · intro g' hg'
      ...
      rw [List.filter_nil, hdt, walkSegO_empty]
...
/-- The O weave's starting state carries the full edge invariant,
given the initial alignment: `wEdge_initP` with the pump half pinned
to `weavePumpsO` — the base lemma hard-codes `weavePumps`, which the
query-first absorber cannot satisfy, and concludes at `weaveInit`;
this twin racks `weaveInitO`'s O pump rows (cf. `wcount_initO`). -/
theorem wEdge_initPO {P : List (List Ev)} {fut : List Ev}
    ...
    WEdgeP sk P fut (weaveInitO sk ord) := by
  refine ⟨wcount_initO sk ord halign hpumps howners, ?_, ?_⟩
  · intro k c n h
    simp [weaveInitO] at h
  · intro k c n h
    simp [weaveInitO] at h

/-- The O weave respects every edge GIVEN pointwise readiness (cf.
`weaveStateE_wedge_of_emitOK`). -/
theorem weaveStateO_wedge_of_emitOK (hwf : sk.wellFormed = true)
    (hemit : EmitOKOnO sk ord
      ((weaveOps sk).flatMap (opEventsO sk ord)) []) :
    WEdgeP sk (procsO sk ord) [] (weaveStateO sk ord) := by
  obtain ⟨hown, halign⟩ := weaveO_initial_alignment sk ord hwf
  have hgo : goEventsO sk ord (weaveFuel sk) (weaveOps sk)
      = (weaveOps sk).flatMap (opEventsO sk ord) :=
    goEventsO_weave sk ord (weave_events_lengthO sk ord hwf)
  have hinit : WEdgeP sk (procsO sk ord)
      (goEventsO sk ord (weaveFuel sk) (weaveOps sk))
      (weaveInitO sk ord) :=
    wEdge_initPO sk ord (by rw [hgo]; exact halign)
      (procsO_drop_pumpsO sk ord) (by rw [hgo]; exact hown)
  have hdep : DepOK [] (goEventsO sk ord (weaveFuel sk)
      (weaveOps sk)) :=
    weaveO_goEvents_depOK sk ord hwf
  ...
  obtain ⟨e₁, opsTail, hops, he₁⟩ :
    ...
    ⟨_, _, rfl, rfl⟩
  have hgo1 : goEventsO sk ord (weaveFuel sk) (weaveOps sk)
      = e₁ :: goEventsO sk ord f opsTail := by
    rw [hfuel, hops]
    rfl
  have hen : enabled sk (weaveInitO sk ord).sent
      (weaveInitO sk ord).rcvd e₁ = true := by
    rw [he₁]
    exact enabled_snd_low sk (cap_pos hwf _)
  have hW1 : WEdgeP sk (procsO sk ord)
      (e₁ :: goEventsO sk ord f opsTail) (weaveInitO sk ord) := by
    rw [← hgo1]
    exact hinit
  show WEdgeP sk (procsO sk ord) []
    (wPump sk (weaveGoO sk ord (weaveFuel sk) (weaveOps sk)
      (weaveInitO sk ord)))
  have hstep1 : weaveGoO sk ord (weaveFuel sk) (weaveOps sk)
        (weaveInitO sk ord)
      = weaveGoO sk ord f opsTail
          (wEmitP sk (weaveInitO sk ord) e₁) := by
    rw [hfuel, hops]
    rfl
  rw [hstep1]
  refine wEdge_pump sk ?_
  refine weaveGoO_wedge sk ord f opsTail _ [e₁]
    (wEdge_emitP sk hen hW1) ?_ ?_ ?_ (wPump_fixpoint sk _)
  · have hd1 : DepOK [] (e₁ :: goEventsO sk ord f opsTail) := by
      rw [← hgo1]
      exact hdep
...
/-- THE O WEDGE, closed: the O weave's final state respects every edge
at the `procsO` family under margin 0 — the class's completeness
witness is edge-respecting at EVERY assignment of the two-point
dequeue-order class. -/
theorem weaveO_wedge (hwf : sk.wellFormed = true)
    (hm0 : ∀ s, sk.dCount s ≤ sk.capLevel) :
    WEdgeP sk (procsO sk ord) [] (weaveStateO sk ord) :=
  weaveStateO_wedge_of_emitOK sk ord hwf (emitOK_weaveO sk ord hwf hm0)
