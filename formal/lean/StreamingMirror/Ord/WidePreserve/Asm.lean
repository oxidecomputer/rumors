/-
Wide-O preservation for the assemblers (`asmRecvRes`/`asmRecvLevel`/
`asmSend`/`asmClose`) over `InvPWO` at every capacity vector κ. The
three non-push arms are capacity-free base arms; `asmSend` destructures
`applyW`'s κ-guard, whose value is never consumed. Each proof is its
Ord/Preserve/Asm.lean twin with the capacity half dropped; the three
`setAsm` O flow frames re-land as private twins.

Chain (ord, stage G): the widened flagship's assembler preservation
cases, consumed by Ord/WidePreserve.lean. Base mirror:
Proofs/Wide.lean (`invPW_preserved_W`'s assembler arms); minus-cap
source: Ord/Preserve/Asm.lean. Map: PROGRESS.md §13.
-/
...
is invisible to every O consumer count (a private twin of
Ord/Preserve/Asm.lean's `recvdOfO_setAsm_of_counts`). -/
...
/-- O flow frame for `asmRecvRes`, away from `asmResChan pk` (a private
twin of Ord/Preserve/Asm.lean's `recvdOfO_setAsm_frame_res`). -/
...
private twin of Ord/Preserve/Asm.lean's `recvdOfO_setAsm_frame_level`). -/
...
/-- `asmClose` wide (a shared, capacity-free arm): phase 3 → 4 with
nothing else moving (cf. `preserve_asmCloseO` minus the capacity
half). -/
theorem preserve_asmCloseWO (_hwf : sk.wellFormed = true)
    (pk : Party × Nat)
    (hstep : applyWO sk κ ax ord (.asmClose pk) s = some s')
...
/-- `asmRecvRes` wide (a shared, capacity-free arm): occupancy on
`asmResChan pk` drops by one exactly as its O consumer count rises
(cf. `preserve_asmRecvResO` minus the capacity half). -/
theorem preserve_asmRecvResWO (hwf : sk.wellFormed = true)
    (pk : Party × Nat)
    (hstep : applyWO sk κ ax ord (.asmRecvRes pk) s = some s')
    (hi : InvPWO sk ax ord s) : InvPWO sk ax ord s' := by
...
/-- `asmRecvLevel` wide (a shared, capacity-free arm): occupancy on
`asmLevelChan pk` drops by one exactly as its O consumer count rises
(cf. `preserve_asmRecvLevelO` minus the capacity half). -/
theorem preserve_asmRecvLevelWO (_hwf : sk.wellFormed = true)
    (pk : Party × Nat)
    (hstep : applyWO sk κ ax ord (.asmRecvLevel pk) s = some s')
    (hi : InvPWO sk ax ord s) : InvPWO sk ax ord s' := by
...
/-- `asmSend` wide: the κ-guarded push (cf. `preserve_asmSendO` minus
the capacity half; the guard value is never consumed). -/
theorem preserve_asmSendWO (_hwf : sk.wellFormed = true)
    (pk : Party × Nat)
    (hstep : applyWO sk κ ax ord (.asmSend pk) s = some s')
    (hi : InvPWO sk ax ord s) : InvPWO sk ax ord s' := by
  have hstep' : applyW sk κ ax (.asmSend pk) s = some s' := hstep
