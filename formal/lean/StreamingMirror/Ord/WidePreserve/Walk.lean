/-
Wide-O preservation for the walk stages: the four order-dispatched
arms (`walkRecvWire`/`walkRecvAsked`/`walkCloseWire`/`walkCloseAsked`)
and the shared `walkCommit`, over `InvPWO` at every capacity vector κ.
The four dispatched arms are `applyWO`'s VERBATIM `applyO` arms
(receives and closes never consult a push guard), so each proof is its
Ord/Preserve/Walk.lean twin with the capacity half dropped — the close
cores and the two prologue-receive cores re-land as minus-cap private
twins, and the dispatch scripts re-run over `applyWO`.

Chain (ord, stage G): the widened flagship's walk-family preservation
cases, consumed by Ord/WidePreserve.lean. Base mirror:
Proofs/Wide.lean (`invPW_preserved_W`'s walk arms); minus-cap source:
Ord/Preserve/Walk.lean. Map: PROGRESS.md §13.
-/
...
3 → 4 with nothing else moving (a minus-cap twin of Ord/Preserve/
Walk.lean's `preserve_close3`). -/
...
/-- The second end-of-stream wait: phase 4 → 5, same frame shape (a
minus-cap twin of Ord/Preserve/Walk.lean's `preserve_close4`). -/
...
/-- `walkCloseWire` wide: phase 3 → 4 (reply-first) or 4 → 5
(query-first); the cores cover both (cf. `preserve_walkCloseWireO`). -/
theorem preserve_walkCloseWireWO (hwf : sk.wellFormed = true)
    (pk : Party × Nat)
    (hstep : applyWO sk κ ax ord (.walkCloseWire pk) s = some s')
    (hi : InvPWO sk ax ord s) : InvPWO sk ax ord s' := by
  cases hord : ord.walk pk <;> simp only [applyWO, hord] at hstep
...
/-- `walkCloseAsked` wide: phase 4 → 5 (reply-first) or 3 → 4
(query-first) (cf. `preserve_walkCloseAskedO`). -/
theorem preserve_walkCloseAskedWO (hwf : sk.wellFormed = true)
    (pk : Party × Nat)
    (hstep : applyWO sk κ ax ord (.walkCloseAsked pk) s = some s')
    (hi : InvPWO sk ax ord s) : InvPWO sk ax ord s' := by
  cases hord : ord.walk pk <;> simp only [applyWO, hord] at hstep
...
/-- The first receive of a scope, either channel: phase 0 → 1 with the
occupancy drop balancing the O count rise (a minus-cap twin of
Ord/Preserve/Walk.lean's `preserve_recv_first_core`). -/
...
embedded `normWalk` provably the identity (a minus-cap twin of
Ord/Preserve/Walk.lean's `preserve_recv_second_core`). -/
...
/-- `walkRecvWire` wide: the first receive when reply-first (phase
0 → 1), the second when query-first (phase 1 → 2, `normWalk`
embedded) (cf. `preserve_walkRecvWireO`). -/
theorem preserve_walkRecvWireWO (hwf : sk.wellFormed = true)
    (pk : Party × Nat)
    (hstep : applyWO sk κ ax ord (.walkRecvWire pk) s = some s')
    (hi : InvPWO sk ax ord s) : InvPWO sk ax ord s' := by
  cases hord : ord.walk pk <;> simp only [applyWO, hord] at hstep
...
/-- `walkRecvAsked` wide: the second receive when reply-first (phase
1 → 2, `normWalk` embedded), the first when query-first (phase 0 → 1)
(cf. `preserve_walkRecvAskedO`). -/
theorem preserve_walkRecvAskedWO (hwf : sk.wellFormed = true)
    (pk : Party × Nat)
    (hstep : applyWO sk κ ax ord (.walkRecvAsked pk) s = some s')
    (hi : InvPWO sk ax ord s) : InvPWO sk ax ord s' := by
  cases hord : ord.walk pk <;> simp only [applyWO, hord] at hstep
...
/-- `walkCommit` wide (a shared, capacity-free arm): nothing observable
to any count changes (cf. `preserve_walkCommitO` minus the capacity
half). -/
theorem preserve_walkCommitWO (hwf : sk.wellFormed = true)
    (pk : Party × Nat) (o : Oblig)
    (hstep : applyWO sk κ ax ord (.walkCommit pk o) s = some s')
    (hi : InvPWO sk ax ord s) : InvPWO sk ax ord s' := by
