...
//! is a shareable link.
//!
//! Immutability is achieved through the `before` codec: every node is stored as its
//! canonical [`before::Clock::encode`] bytes. Applying an op decodes a fresh value from a
