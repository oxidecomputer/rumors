...
impl Dial for TcpDial {
    ...
    /// Connect with the requested send buffer and disable Nagle's algorithm.
    async fn dial(&self, addr: &SocketAddr) -> io::Result<TcpStream> {
        let conn = match self.send_buffer {
...
impl Listen for TcpListen {
    ...
    /// Accept a TCP connection and disable Nagle's algorithm.
