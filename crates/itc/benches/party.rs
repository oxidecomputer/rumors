...
fn bench_is_disjoint(c: &mut Criterion) {
    ...
    for &n in SIZES {
        ...
        let imp = common::impl_parties(&plan, 2);
        let orc = common::oracle_parties(&plan, 2);
        g.bench_with_input(BenchmarkId::new("itc", n), &imp, |b, p| {
...
fn bench_partial_cmp(c: &mut Criterion) {
    ...
    for &n in SIZES {
        ...
        g.bench_with_input(
            BenchmarkId::new("itc/ancestor", n),
            &(anc, child),
            |b, (a, c)| {
        ...
        g.bench_with_input(BenchmarkId::new("itc/equal", n), &ieq, |b, (a, c)| {
            b.iter(|| black_box(a.partial_cmp(c)));
        });
...
fn bench_codec(c: &mut Criterion) {
    ...
    for &n in SIZES {
        ...
        g.bench_with_input(BenchmarkId::new("itc/encode", n), &party, |b, p| {
