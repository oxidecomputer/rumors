...
fn version_and_party(
    r: &mut StdRng,
    n: usize,
) -> (Vec<u8>, Party, itc::oracle::Version, itc::oracle::Party) {
...
fn bench_batch(c: &mut Criterion) {
    ...
    for &k in &[1usize, 4, 16, 64] {
        g.bench_with_input(BenchmarkId::new("itc/batched", k), &bytes, |b, bytes| {
            b.iter_batched(
                || Version::decode(bytes).unwrap(),
                |mut v| {
                    {
        ...
        g.bench_with_input(BenchmarkId::new("itc/unbatched", k), &bytes, |b, bytes| {
            b.iter_batched(
                || Version::decode(bytes).unwrap(),
                |mut v| {
                    for _ in 0..k {
...
fn bench_partial_cmp(c: &mut Criterion) {
    ...
    for &n in SIZES {
        ...
        for (kind, ia, ib, oa, ob) in [
            ...
            g.bench_with_input(
                BenchmarkId::new(format!("itc/{kind}"), n),
                &(ia, ib),
                |b, (a, c)| {
...
fn bench_codec(c: &mut Criterion) {
    ...
    for &n in SIZES {
        ...
        g.bench_with_input(BenchmarkId::new("itc/encode", n), &version, |b, v| {
