...
fn world_strategy() -> impl Strategy<Value = Vec<Op>> {
    prop::collection::vec(op_strategy(), 0..30)
}

/// Apply a trace to a population that starts as a single seed clock.
fn run(ops: &[Op]) -> Vec<Clock> {
    let mut cs = vec![Clock::seed()];
    for op in ops {
        let n = cs.len();
        match *op {
...
fn versions(cs: &[Clock]) -> Vec<Version> {
    cs.iter().map(|c| c.version()).collect()
}

/// `a <= b` under the causal order (treating concurrency as not-`<=`).
fn leq(a: &Version, b: &Version) -> bool {
