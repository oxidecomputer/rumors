...
///
/// The offset/sum fields are arbitrary-precision [`Base`] path sums, so the
/// struct is `Clone` (not `Copy`); helpers clone rather than copy.
#[derive(Clone)]
pub(super) struct Side {
    ...
    pub(super) pos: usize,
    /// This side's pinned offset (path sum down to it), reused when it is a
    /// leaf.
    pub(super) off: Base,
    /// This side's node sum (`off + base`), the offset carried into a threaded
    /// child.
    pub(super) sum: Base,
...
impl Side {
    /// Where this side's left child starts: descend if a node (its
    /// `next`/`sum`), else re-broadcast the leaf in place (its pinned
    /// `pos`/`off`).
    ...
    /// this side descended, else the same re-broadcast leaf.
...
/// `version/` (compare, event, grow) and nowhere outside it.
