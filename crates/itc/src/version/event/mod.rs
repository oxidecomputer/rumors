...
//!   root it produced (`join`'s register, [`Built`] shared by `fill` and the grow emit),
//!   the subtree's cost (`grow`'s `Probed`), or nothing (`compare`'s `Ends`).
...
//! (`sum` in `party::ops` plays the same role with an output builder plus its `Summed`
//! register, since it must also finalize normalized output nodes.)
//!
...
pub(crate) fn tick(id: &BitsSlice, ev: &WorkingVersion) -> WorkingVersion {
    if id.len() == 2 && !id[0] && id[1] && ev.base.len() == 1 && !ev.topo[0] {
        let mut out = Builder::with_capacity(1);
        out.leaf(ev.base[0].clone() + 1u32);
        return out.finish();
    }
