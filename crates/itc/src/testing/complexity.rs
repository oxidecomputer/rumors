...
//!
//! Each measured traversal is wrapped in [`steps_of`] (a fresh [`super::metrics`] counter),
//! and the two results — at input sizes whose node counts differ by `4×` — are fed to
//! [`assert_linear_scaling`], which fails if step growth looks quadratic rather than linear.
