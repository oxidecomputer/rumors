...
fn main() {
    ...
    let new = format!(
        "{head}{marker} the constants below, as `rustc --version`\n\
         /// reports it. Generated by `just fuzzfit-calibrate` alongside [`BANDS`]:\n\
         /// guest codegen (and so every fuel constant) is a function of this\n\
         /// compiler, and the suite asserts the building toolchain matches, so a\n\
         /// toolchain bump reads red until the bands are re-pinned. wasmtime (the\n\
         /// fuel schedule's other half) is pinned exactly by the workspace\n\
