...
| `Rank`/`Ranked` | a total order extending the causal order       | `Ord` (`<`, `==`, `>`, etc.), summation (`+`), `checked_sub`                                                                               |
...
Rust, `before` is between 2–20× faster (measured: the workspace
...
- **`serde`** — `Serialize`/`Deserialize` for `Party`, `Version`, and
  ...
  same strict validation as `decode`.
