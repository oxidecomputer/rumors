...
impl Clock {
    ...
    ///
    /// The iterator yields exactly `k` children and `self` keeps a residual
    /// share, so it stays valid after the iterator is fully drained. Forks not
    /// taken before the iterator drops have their party shares rejoined into
    /// `self`, so no [`Party`] is lost.
    ///
    ...
    /// use before::Clock;
    /// let mut parent = Clock::seed();
    /// let children: Vec<Clock> = parent.forks(3u64).collect();
