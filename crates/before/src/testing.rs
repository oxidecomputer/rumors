...
//! strategies ([`generators`]), the deterministic RNG ([`rng`]), the op-trace
//! history generator ([`optrace`]), the brute-force grow-optimality
//! reference ([`grow_brute_force`]), and the shape-walk row vocabulary
//! ([`shape_rows`]).
//!
...
//! prod↔tree↔fs differential-leg roster; "which suite covers operation X"
//! starts there).
//!
//! Part of the same architecture, outside this module: the recursive reference
//! implementation the legs compare against ([`crate::oracle`]), the law
//! predicates ([`crate::laws`]), the per-production-module unit tests in their
//! `*/tests.rs` siblings, and the out-of-process suites in `tests/` (the
//! resource-envelope pins, the board smoke test, the bench-judge membership
//! pins, the fuzz seed corpus). Compiled only under `cfg(test)`; never part of
//! the shipped crate. The map of how every validation instrument — in this
//! module and out of it — fits together, written for a maintainer orienting
//! cold, is [`validation_index`].
