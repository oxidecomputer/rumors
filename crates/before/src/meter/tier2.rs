...
//! are derivable from the absolute leaf values and store nothing. The
//! compactness ratio between this size and today's encoded size is the
//! evidence the representation decision turns on, so the walk here is written
//! for obvious correctness over economy: one preorder pass over the packed
//! form, absolute leaf values accumulated as root-to-leaf path sums in
//! arbitrary-precision arithmetic (the crate's `Base`, so no magnitude
//! saturates or overflows the measurement).
//!
//! The zigzag map is the canonical sign convention `k >= 0 -> 2k`,
...
/// `nodes` flag bits plus its stored gamma codes, so the stored-base code
/// bits are exactly `encoded_bits - nodes`).
...
/// no stack-growth guard at any input depth.
///
/// # Panics
///
...
pub fn tier2_size(version: &Version) -> Tier2Size {
    ...
    // of an internal node inherit the same sum, and the stream lists the
    // whole left subtree before the right, so a plain stack stays aligned.
