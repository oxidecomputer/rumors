...
//! rationale ([`Liveness`]). There is no default, no `..` construction, and no
//! wildcard to hide behind — a currency cannot be half-wired, which is the
//! genre of gap (work migrating into an unjudged currency on rows that never
//! carried it) this axis exists to make inexpressible. [`ByCurrency::each`]
//! destructures exhaustively for the same reason: the judgment and render loops
//! iterate the axis itself, so a new currency is judged and rendered everywhere
//! or the destructuring fails to compile.
...
/// One value per metering currency: the container the currency axis distributes
/// over the board's cells.
///
/// Constructed only by exhaustive struct literal (no `Default`, no `..`): see
/// the module doc's totality argument.
...
impl<T> ByCurrency<T> {
    /// Every currency's value, in the axis's fixed judgment and render order.
    ///
    /// The exhaustive destructure is deliberate: a currency added to the axis
    /// fails to compile here until it joins the iteration, so no judgment or
    /// render loop can silently skip it.
...
/// One judged column's liveness declaration for one cell: the least the counter
/// must read if the meter is watching the work, or the reason no floor can
/// bind.
///
...
///
/// Constructing a board cell requires answering the floor question for every
/// currency on the axis — a cell cannot enter the board without the answers,
/// and a currency cannot enter the axis without every cell's answer (the module
/// doc's totality argument). Segments' declaration is the ceiling-only policy
/// NA on every cell today: the target is walks that never grow the stack, so
/// its honest floor is zero, and a zero floor asserts nothing.
