...
//! pre-scans (`PreScan`) — memoized: one fresh scan records every interior
//! left-full site's minimum as a frame-ledger link (the `Memo` doc), so no
//! stream position is ever pre-scanned twice and no minimum is materialized.
//!
...
//! already holds when it arrives (the previous sibling site's minimum, the
//! forest parent's, or the scan-entry height), never an absolute — folded into
//! the live relation exactly once, at the raise decision it serves.
//!
//! # Cost
//!
...
//! families that separate this from every refuted discipline]. The cost
//! invariant conserves width: every touch is paid by a consumed input code, an
//! emitted output code, or the death of the digits it reads. Each consumed
//! delta folds into O(1) accumulators — and a region the id owns nothing under
//! is consumed as one block, its leaves folded into a net movement and a
//! streaming minimum whose every fold is priced by the code the block scan just
//! read, the whole region then entering the walk's registers and the watermark
//! web as one delta and one emission (the `tick_ownership_hole` envelope pins
//! the block scan engaging; the `tick_ownership_comb` envelope pins the gate
//! free when regions are too small to open it); each emission's watermark
//! update is one amortized sign read plus a propagation whose every fold is a
//! dying operand or the one surviving fold the update's own priced width
//! bounds; a close moves its popped boundary into the watermark stack's latent
//! register and an arm recycles the register into the new boundary, so the
//! close-reveal cycle's wide content shuttles by moves at a narrow
//! anchor-relative marginal cost (the `watermark` module doc carries the
//! register's discipline); each emitted code is materialized once,
//! post-collapse, at its own width; the watermark compares fold and restore
//! only the priced offset (or answer post-sign by top-index domination, through
//! the latent ladder where one is parked); the extremum scans' reset-on-cross
//! folds are priced by the range they scan; the absent-sibling raise compares
//! materialized offsets both priced by their own scans; the builder's
//! equal-sibling seam is a one-bit code check. The ledger's own operands obey
//! the same lifetime rules: each link is created once at its site's close (the
//! head moves into the queue, or a deferred first-child link is cloned once at
//! its own width), read once at its consume, and dies into the raise decision;
//! the keeper and suspend folds each read a dying operand or a link's own
//! priced width; the one live follower (the head, walk-side the relation)
//! receives the per-event fold the watermark discipline already prices — closes
//! excepted, where it rides the latent tag untouched — and the pre-scan's
//! recorder adds one amortized sign read per site (the zero-link test). Wide
//! content is read only where an operand dies, a bounded-count lifetime read,
//! or a code prices it — the height↔watermark anchor switches read the
//! surviving web once, priced by the switch emission's own code, with a parked
//! latent cancelling symbolically on the watermark-to-height switch and
//! retiring on the other.
//!
...
//! control bits plus pop-able word deltas (`Frames` and `PreFrames`, the route
//! fold's own `PopStack` discipline) — so paired depth costs a few heap bits
//! per level, never a call-stack frame, and no input depth can grow stacker
//! segments or overflow. The wide quantity a left-full site's raise decision
//! needs after its sibling walk is re-derived by one bounded replay of the
//! site's own collapse range (`PreScan::replay_max`) rather than parked per
//! open site, so frames stay word-free and the transient stays flat on
//! nested-site chains.
//!
//! # Testing
//!
...
/// The memoized pre-scan's output — the frame ledger: per left-full site, in
/// the walk's arrival (pre-order) order, one optional link resolving the site's
/// minimum against a reference the walk already holds when it arrives.
///
/// The reference discipline: a site with an earlier sibling under the same
/// forest parent stores `m_s − m_prev`; a forest parent's first child stores
/// `m_s − m_parent`, written at the parent's own close — when the parent's
/// minimum is final — into the child's earlier queue slot (the queue is written
/// out of order, consumed in order); the scan's outermost site stores `m_root −
/// h(scan entry)`. The walk's arrival-order relation is exactly that reference
/// at every consume: the site before a first child is its parent (the live
/// relation), and the site before a later sibling is the previous sibling
/// (re-anchored from the walk's own web at that sibling's range close). Zero
/// links are not stored at all, so sibling or nested sites sharing one minimum
/// cost nothing — one wide shared minimum is never materialized per covering
/// site. Each stored link is created once, read once at its consume, and dies
/// into the raise decision.
struct Memo {
    /// Per site, in consumption (stream) order: 0 when the site's link is zero,
    /// else the 1-based index of its link in `links` — one machine word per
    /// site, so sites sharing minima store nothing beyond it.
    queue: Vec<u32>,
...
impl FillWalk<'_> {
    ...
    fn consume_payload(&mut self) -> Signed {
        ...
        fold_signed_int(&mut self.h, neg, &mag);
        self.stack.fold_height(neg, &mag);
        if !self.w_anchored {
            fold_signed_int(&mut self.gap, neg, &mag);
        }
    ...
    /// [`consume_site`](Self::consume_site) with a height-carried relation `acc
    /// = h − m_r`.
    ...
    /// body outright.
    fn emit_step(&mut self, depth: usize, neg: bool, mag: Int) {
        ...
        if !self.started {
            ...
            self.started = true;
            self.gap.reset();
            self.out.leaf(depth, gamma_code_int(&mag));
            return;
        }
        let _ = (neg, mag);
    ...
    fn copy_subtree(&mut self, depth: usize) {
        ...
        // consumed either way. `d1 < 2` (a lone leaf, or a leaf-first pair
        // whose left half is one leaf) stays per-leaf: under a finely
...
/// keys against a running register; deferred left costs), the route fold's own
/// [`PopStack`] discipline — so a deep spine costs a few heap bits of transient
/// per level, never a machine-word frame.
struct Frames {
    ...
    aux: BitStack,
    /// Key deltas (one per frame, against `reg`) and deferred left costs (two
    /// components per left-to-right flip), LIFO with the frames they serve.
    vals: PopStack,
    /// The top frame's route key (id positions only advance, so every pushed
    /// delta is nonnegative and a pop restores by subtraction).
    reg: usize,
}
...
/// sibling.
///
/// Computes every interior left-full site's `min(fill(ir, er))` on its own
/// watermark web and records each as a frame-ledger link (the [`Memo`] doc), so
/// the walk arrives with every raise argument resolved and no position is
/// pre-scanned twice.
///
/// The image of the fill equations restricted to the minimum (each arm derived
/// from the oracle's):
///
...
/// per-range net movements are never materialized.
struct PreScan<'a, 'm> {
    ...
    ///
    /// Per entry: the outer head's final value (`m_first(inner) − m_ref(outer)`
    /// — immutable once pushed, both minima fixed), the outer keeper, the outer
    /// first slot, and the outer level. LIFO by the site forest's nesting.
    suspend: Vec<(Accumulator, Accumulator, usize, u32)>,
}

impl PreScan<'_, '_> {
    /// The pre-scan image of the walk's arms over the subtree at `pos`: same
    /// reads, virtual emissions; returns the range end.
    ///
    /// The iterative twin of [`FillWalk::walk`]: the descend phase resolves the
    ...
    fn resolve_inner(&mut self) {
        // x := (min − m_last) + (m_last − m_first) = min − m_first; the keeper
        // dies into it (its buffer is re-armed for the outer level below —
        // nothing is minted per resolve).
        ...
        // The fold runs NARROW side INTO wide survivor: x dies at the link's
        // own funded width (zero when the minima are shared), while the
...
/// What the pre-scan still owes a suspended node (the [`Frame`] kinds, minus
/// costs — the pre-scan folds no route).
enum PreFrame {
...
/// The pre-scan's suspended ancestors, held as bits: [`Frames`]' shape with
/// site payloads (ledger slot, collapse-range start) as pop-able word deltas in
