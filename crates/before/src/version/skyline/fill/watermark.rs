...
//! them. The representation is one signed accumulator `t = h − A`
//! (`h` the walk's running input height, `A` an *anchor* at or above
//! the innermost armed range's minimum `m`), one optional latent
//! boundary `Λ = A − m` (strictly positive when present; absent means
//! `A = m` exactly), and a stack of nonnegative differences
...
//!
//! The cost discipline conserves *width*, not just object lifetimes:
//! every digit touch is paid by a consumed input code, an emitted
//! output code, or the death of the digits it reads — so wide content
//! can shuttle between the difference stack and the latent register by
//! moves alone, and no schedule of arms and closes re-reads a width the
//! input paid for only once. Enforced by shape:
//!
...
//! the memoized pre-scan's last recorded minimum). Arms, undercuts, and
//! collapses fold the same operand they already price into each active
//! follower; closes touch no follower at all — each active slot goes
//! *anchor-relative* under a one-bit tag (`f_true = f_stored − Λ`),
//! resolved at the follower's own death: symbolically where the
//! consumer is itself anchor-relative or the switch's terms cancel, by
//! one latent fold where an emitted code prices it, and by the
//! death-event fan-out at undercuts and collapses. A set tag never
//! outlives its latent.
...
impl MinStack {
    ...
    /// already reflects every inner emission (propagation kept it live)
    /// — and the latent makes it O(1): a popped zero run decrements; a
    /// popped nonzero boundary MOVES into the latent register (minting
    /// it, or dying min-into-max into a live one), leaving `t` and the
    /// followers untouched. Each active follower goes anchor-relative
    /// by its one-bit tag instead of absorbing a fold, so a close never
    /// touches a follower digit. The last armed frame's close retires
    /// the web: `t` and any latent drop unread (followers are already
    /// dead, so no surviving relation needs re-anchoring).
    pub(super) fn close(&mut self) {
        ...
        match self.diffs.pop().expect("armed > 1 has a difference record") {
            ...
            DiffEntry::Diff(d) => {
                ...
                // the anchor stays where it is and the boundary parks
                // in the latent (`Λ += d` by min-into-max merge, or the
                // mint move). Active followers were exact against the
                // old anchor state, so tagging them anchor-relative is
                // value-preserving: a mint finds them `m`-exact with
                // `A = m_old`, a merge finds them already tagged (a
                // live latent keeps every active follower tagged).
    ...
    /// width widens, or a re-anchor riding one (the site close and
    /// recorder paths, whose in-cycle case finds the latent already
    /// drained by the arm).
    ...
    pub(super) fn emit_here(&mut self) {
        ...
        // v − A = t: one amortized sign read; at or above the anchor is
        // at or above the minimum and costs nothing further.
    ...
    pub(super) fn emit_offset(&mut self, off: &Signed) {
        ...
        // v − A = t + off. With no latent, post-sign domination decides
        // against a word-scale offset with no fold (with one live, the
        // O(off) fold below is cheap for a word and the ladder decides).
        if self.latent.is_none() && off.1.to_u64().is_some() {
    ...
    /// Decide a drop below the anchor (`t < 0` holding `v − A`) against
    ...
    /// changes; a dominated one means a true undercut — return true
    /// with the latent left live for [`undercut`](Self::undercut)'s
    /// residue to annihilate. Comparable tops retire the latent (the
    ...
    /// folded and restored, with the latent ladder deciding drops that
    /// land between the true minimum and the anchor (domination in
    /// O(1), or a funded collapse at comparable scales).
    pub(super) fn compare_above(&mut self, above: &Signed) -> Ordering {
        ...
        if self.latent.is_none() && above.1.to_u64().is_some() {
            let (sign, decided) = self.t.sign_dominates_word();
            if decided {
                return sign;
            }
    ...
    /// anchor (`A + d_arm`, `d_arm` signed): `Ordering::Less` means
    /// strictly below it.
    ///
    /// The memo consumer's decision read: `(h + above) − (A + d_arm) =
    /// t − d_arm + above`, folded and restored — `above` is priced,
    /// `d_arm` is anchor-relative dying content (a ledger link net of
    /// the taken relation, narrow whenever the reference minima agree),
    ...
    /// Arm the pending frame at a minimum `d_arm` above the anchor
    /// (`v = A + d_arm`, `d_arm` signed and dying here).
    ///
    /// The memo consumer's arming: the new `t = t_old − d_arm` needs no
    /// read of the old web beyond `d_arm`'s own width, and `d_arm`
    /// recycles any parked latent — the true boundary `v − m` is
    ...
    ///
    /// While a latent lives the caller's content must be
    /// anchor-relative (`A − X`) — the slot is tagged and reads resolve
    /// through the latent. Every install site either derives its
    /// content from the anchor web itself (already anchor-relative) or
    ...
    ///
    /// The content is anchor-relative when the slot was tagged (a
    /// latent lives): `f_true = f_stored − Λ`. Callers either consume
    /// it against another anchor-relative quantity (the tag cancels
    ...
    /// watermark-to-height anchor switch's bridge read, priced by the
    /// code emitted at the switch that needs it.
    ///
    /// Deliberately anchor-relative: the caller's `acc` is a follower
    ...
    fn arm(&mut self, below: Accumulator) {
        ...
        // The anchor-relative offset: d = v − A_old = t_old − below.
        // t_old dies into it (a move of the buffer, one narrow fold),
        // and the boundary bookkeeping recycles any parked latent.
