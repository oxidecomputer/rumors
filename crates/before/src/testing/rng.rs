...
///
/// Backed by [`ChaChaRng`], whose output is documented-portable across
/// platforms (unlike `rand`'s `StdRng`, which explicitly reserves the right to
/// change), and seeded through `seed_from_u64`'s documented expansion, so a
