...
impl<'a> IdReader<'a> {
    ...
    /// walk: the two halves of the disjoint union, or `None` if the ids
    /// overlap.
    ///
    /// Byte-identical to [`sum`](IdReader::sum) followed by
    /// [`split`](IdReader::split) of the union, with the union itself never
    /// built.
    ///
    ...
    /// both-present node (the *branch*) per half. Both structures are visible
    /// in the operands directly:
    ///
    ...
    /// twice. When the *left* child pair is a genuine merge, positioning a
    /// both-present operand's right child would mean skipping its left child
    /// and then reading it again inside the merge — paying more than the
    /// composition wherever the union collapses — so the walk delegates the
    /// whole branch subtree pair to `sum` and `split` (both cursors still sit
    /// at the branch tags, and the branch subtrees are suffixes of their
    /// streams): the composition's own bytes and reads, minus the built union's
    /// spine. Everywhere else the targeted path runs, and every skip it pays
    /// feeds a splice.
    ///
    /// The one seam `split` sees that the targeted path does not is the union
    /// collapsing *at* the branch: when both union children are full, the built
    /// union's branch node becomes a terminal and `split` lands in its terminal
    /// arm — which emits per half a one-child node over a terminal, exactly the
    /// tag-plus-full-child bytes the branch arm here emits. The
    /// `sum_split_is_sum_then_split` differential holds the whole map, `None`
    /// arm included, to the composition.
    ///
    /// Overlap is detected exactly as [`sum`](IdReader::sum) detects it — a
    /// full leaf meeting a nonempty region, on the spine or inside a delegated
    /// merge — and nothing is returned partially built.
    ///
    /// `O(|self| + |other|)` worst case, never above the composition's own
    /// reads, and sublinear where the operands do not interleave: a subtree
    ...
    pub(crate) fn sum_split(mut self, mut other: IdReader) -> Option<(BitsMut, BitsMut)> {
        ...
        loop {
            ...
            let (al, ar) = match a_node {
                // A full leaf meets the other id's nonempty region (the other
                // cursor is live, so its subtree here is nonempty): the ids
                // share a region, and there is no disjoint union.
            ...
            if left && right {
                if al && bl {
                    // The left pair is a genuine merge: delegate the whole
                    // branch subtree pair to the composition (the method doc's
                    // mode argument). The cursors still sit at the branch tags,
                    // and the branch subtrees are suffixes of their streams, so
                    // `sum` merges exactly them; its root is both-present or
                ...
                // retag, and its own side's union child.
                self.read();
                other.read();
            ...
            // A unary union node: both operands are unary in the same direction
            // (the method doc's spine argument), and the tag rides into both
            // halves.
            self.read();
            other.read();
            spine.push(left);
...
/// The bit ranges of one operand's children at the union's branch node, with
/// the cursor just past the node's tag.
///
/// The branch node's subtree is a suffix of its stream (the spine descent above
