...
//! interval, so it rides the event side's overlay-advance law itself, through
//! the plateau-cursor trait (the skyline sweep module states the law once and
//! carries its boundary bookkeeping): two cursors walk the operands' overlay
//! partition, the output — owned exactly where `self` is and `other` is not —
//! is appended item by item at the deeper cursor's depth, and the collapsing
//! output builder ([`IdSkylineBuilder`]) re-derives the canonical id from the
//! depth sequence. What the event sweep's accumulator does for integer heights,
//! a single owned bit per cursor does here: the boolean semiring needs no
//! running difference, only the two current values.
//!
//! # Covered blocks
//!
//! Because dyadic intervals nest, whenever one cursor is about to enter a
//! subtree, the other cursor's current plateau either covers that subtree's
//! whole interval or subdivides it. A covering plateau makes the output over
//! the interval a *block*, settled without walking the subtree plateau by
//! plateau:
//!
...
//!
//! The fourth pairing (`self` owned over an `other` subtree) is not a block:
//! the output there is `other`'s complement, so the sweep walks that subtree
//! plateau by plateau. When neither cursor's plateau covers the other's subtree
//! — both enter subtrees over the same interval — the two descend in lockstep
//! until a plateau or a covered block appears ([`descend_pair`]).
//!
//! Nothing recurses and nothing is read twice: every topology tag of either
//! operand is read at most once (block scans consume the tags their subtree
//! would have spent on descent), the transient state is bits per open ancestor
//! (two cursor paths and the builder's stacks), and identical deep operands —
...
impl IdReader<'_> {
    ...
    /// ([`Party::without`](crate::Party::without)) maps that empty result to
    /// `None`, since a `Party` is a nonzero share.
    ///
    /// The result is always a subregion of `self` (`self \ other ⊆ self`), so
    /// it introduces no region `self` did not already own. That is what keeps
    /// it linearity-safe where a general id *meet* is not (see the note on the
...
/// a tie (it debug-asserts the shared flip level); each crossing it yields is
/// that cursor's *entering* flag. A cursor stepping *alone* lands strictly
/// deeper than the other's current plateau, so that plateau covers whatever
/// subtree it entered (dyadic intervals sharing a point nest): the solo arms
/// settle against it directly. A tie lands both cursors on slots of one shared
/// depth, settled jointly by [`settle_pair`].
...
/// Settle a `self` subtree that `b`'s current plateau covers: nothing of it
/// survives an owned cover (a block scan, one unowned plateau), and all of it
/// survives an unowned cover (a block scan, one verbatim splice).
///
/// The caller guarantees the cover: `b`'s plateau contains the subtree's start
/// and is at most as deep, so by dyadic nesting it contains the whole subtree
/// interval.
...
///
/// Under an unowned `a` there is nothing to carve from (a block scan, one
/// unowned plateau), while under an owned `a` the output is the subtree's
/// complement, so the sweep must walk it plateau by plateau (an eager descent,
/// no block).
///
...
/// Settle both cursors after a shared-depth boundary: whichever side stepped
/// into a subtree (`*_entering`) is settled against the other.
///
/// Both slots sit at one shared depth, so a side that settled at a plateau (its
/// slot was an absent child) covers the other side's subtree; when both entered
/// subtrees over the same interval, neither covers, and the pair descends in
/// lockstep.
...
/// Descend two subtrees over the same interval in lockstep until one side
/// reaches a plateau or an absent child, then settle the other side against it.
///
/// Each round consumes both tops. While both are internal with present left
/// children the descent continues one level down (the intervals stay equal);
/// the moment one side resolves to a plateau — a terminal (owned at the shared
/// depth) or an absent left child (unowned one level down) — that plateau
/// covers whatever the other side still has open at or below the same depth,
/// and the covered-block rules of [`settle_a`]/[`settle_b`] apply.
...
/// The cursor's current item: one entry of its side of the overlay tiling,
/// occupying the dyadic interval at the cursor's depth.
#[derive(Clone, Copy)]
enum Item {
    /// A plateau: owned (a terminal) or unowned (an absent child, or a subtree
    /// consumed as a covered block that contributes nothing).
    Plateau { owned: bool },
...
enum Enter {
    /// The top was the terminal: the cursor is at an owned plateau at the same
    /// depth.
    Full,
    /// The top was internal with a present left child: the cursor is atop that
    /// subtree, one level deeper, still unsettled.
    Left,
    /// The top was internal with an absent left child: the cursor is at an
    /// unowned plateau, one level deeper.
    Absent,
}
...
/// consumed forward at most once, the root-to-item path is the only per-depth
/// state, and the same three dyadic facts (the deeper item ends first; ties
/// close to one shared flip level; the all-right path is the exhausted tiling)
/// drive [`advance`]. An absent child — a stored `0` occupies no bits — is
/// presented as a synthetic unowned plateau at the child's own depth, so the
/// cursor always tiles the whole interval.
///
/// Stepping into a present subtree leaves the cursor *unsettled* (the step
/// reports it): the sweep decides — from the other operand's current plateau —
/// whether the subtree is consumed as one covered block
...
impl<'a> IdLeafCursor<'a> {
    /// Open an id at its root, unsettled atop the whole tree (`true`) for the
    /// sweep to settle against the other operand.
    ///
    /// A synthetic [`Empty`](IdReader::Empty) reader is the anonymous `0` id —
    /// one unowned plateau covering the whole interval, already settled
    /// (`false`).
    ...
    /// Whether the current plateau is owned. Never queried on a spliced block:
    /// the splice's covering side is always a plateau.
    fn owned(&self) -> bool {
        match self.item {
    ...
    /// ([`Enter::Full`]); an internal node opens the level and lands on its
    /// left slot — settled at the synthetic unowned plateau if the left child
    /// is absent ([`Enter::Absent`]), unsettled atop the left subtree if
    /// present ([`Enter::Left`]).
    ...
    /// Descend from an unexplored subtree top to its first plateau: read each
    /// internal tag and enter its left slot, stopping at a terminal or an
    /// absent left child.
    fn descend(&mut self) {
        while matches!(self.enter(), Enter::Left) {}
    }

    /// Consume the whole unexplored subtree at the cursor as one covered block,
    /// without walking its plateaus.
    ///
    /// A terminal top settles as the owned plateau (the sweep's pointwise
    /// emission already yields the block's constant there); an internal top is
    /// scanned past in one iterative pass (the shared
    /// [`skip_subtree`](crate::idbits::skip_subtree) discipline: a
    /// pending-children counter, never the call stack) and stands as a verbatim
    /// [`Item::Splice`] when it survives into the output (`splice`, an unowned
    /// `other` cover) or as one unowned plateau when nothing of it does (an
    /// owned `other` or unowned `self` cover).
...
impl PlateauCursor for IdLeafCursor<'_> {
    /// A step's crossing is its *entering* flag: whether the cursor is now
    /// unsettled atop a present subtree.
    ///
    /// An absent slot settles immediately as the synthetic unowned plateau. The
    /// sweep settles every entered subtree against the other operand before
    /// emitting.
    type Crossing = bool;
    ...
    /// Advance past the current item to the next slot: the flip level's depth
    /// for the law's tie test, and the entering flag.
    ///
    /// Pops the trailing right-branch levels (each an ancestor whose subtree
    /// the consumed item completed), then steps the deepest left-branch level
    /// to its right child slot.
    ///
