//! A per-call random-access index over one fixed id operand, for folds
//! that test many independent operands against it.
//!
//! The packed id coding has no random access: locating a node's right
//! child means scanning past its whole left subtree, so a cursor-walk
//! disjointness test costs `O(n + m)` in *both* operands. That is the
//! right shape for one call, and the wrong shape for a fold: testing
//! each of `k` inputs against one fixed operand by cursor re-walks the
//! fixed side `k` times — `Θ(k × n)` on a `Θ(n + Σ inputs)` operand set.
//!
//! [`IdIndex`] restores the missing primitive for exactly that pattern.
...
//! differentials in `party/tests.rs` and `clock/tests.rs` pin the
//! equivalence — and it exists *only* because the fold repeats the
//! test against one fixed side; single-shot predicates stay on the
//! cursor walk, which needs no table.
...
/// operands. Transient state is at most one `u32` per both-present node of
/// the indexed operand — strictly smaller than the operand itself.
pub(crate) struct IdIndex<'a> {
    ...
    /// both-present node in preorder. Only both-present nodes need an
    /// entry: a right-only node's child follows its tag directly, and a
    /// left child always does.
    ///
    /// `None` when the stream's positions do not fit `u32` (≥ 512 MiB of
    /// packed id); [`is_disjoint`](IdIndex::is_disjoint) then falls back
    /// to the per-input cursor walk, which answers the same predicate at
    /// the fold's unindexed cost.
    rights: Option<Vec<u32>>,
}

impl<'a> IdIndex<'a> {
    /// Index `bits` (a normal-form packed id stream) in two linear
    /// passes: count the both-present nodes to size the table exactly,
    /// then fill each node's right-child position.
    ///
    /// An id stream is 2-bit presence tags in preorder and nothing else,
    /// so the counting pass is a flat pair scan. The filling pass
    /// resolves each both-present node's left-subtree end with a stack
    /// of open both-present frames — single-child nodes are transparent
    /// (their completion is their child's) — threading the frames'
    /// entry slots through the not-yet-filled table itself, so the only
    /// transient state beyond the table is one `bool` per open frame.
    pub(crate) fn build(bits: &'a BitsSlice) -> IdIndex<'a> {
        ...
        // Pass 2: fill. Reads every tag once more. While a frame awaits
        // its left subtree's end, its table slot holds the entry index of
        // the next-outer awaiting frame (`u32::MAX` terminates the
        // chain); the real right-child position overwrites the link the
        // moment the left subtree completes.
        crate::codec::scan::record_bits(bits.len());
        let mut rights = vec![0u32; count];
        // One bit per open both-present frame, innermost last: `true`
        // while the frame awaits its left subtree's end, `false` while it
        // awaits its right's.
        ...
        while p < bits.len() {
            ...
            if left && right {
                ...
                // through every frame it finishes. Single-child nodes
                // between frames queued nothing, so their completion is
                // exactly this one.
                loop {
                    match awaiting_left.last_mut() {
                        None => break,
                        Some(state @ true) => {
                            // The innermost frame's left subtree ends
                            // here, so its right child starts here.
                        ...
                        Some(false) => {
                            // The frame's right subtree ends: the whole
                            // frame completes, finishing one child of the
                            // frame outside it.
                            awaiting_left.pop();
                        }
                    }
    ...
    /// Whether the indexed operand and `other` (both normal-form ids)
    /// share no owned region: the same verdict as
    /// [`IdReader::is_disjoint`] on the same pair, in `O(other)` node
    /// visits.
    ///
    /// The walk is driven entirely by `other`'s cursor: a pair is visited
    /// only where `other` has a present node, an indexed-side child is
    /// addressed in `O(1)` through the table, and an indexed-side subtree
    /// standing against an absent `other` child is skipped by never being
    /// visited. Where the *indexed* side is absent, `other`'s subtree is
    /// skip-scanned once to resync its cursor — cost charged to `other`.
    /// Iterative: the per-ancestor state is a queued right-pair stack,
    /// bounded by `other`'s depth, so a deep operand cannot overflow the
    /// call stack.
    pub(crate) fn is_disjoint(&self, mut other: IdReader) -> bool {
        ...
        // The current pair's indexed side: the bit position of its
        // present node, or `None` for an absent (empty) region.
        let mut node: Option<usize> = (!self.bits.is_empty()).then_some(0);
        // The table index of the first entry at or after the current
        // node — the node's own entry whenever it is both-present.
        let mut entry = 0usize;
        // Queued right pairs, innermost last: the indexed side's position
        // (`None` = absent child) and its entry bound. `other`'s right
        // children need no bookkeeping — its cursor reaches each one in
        // stream order, exactly as in the cursor walk.
        ...
        loop {
            ...
            match node {
                ...
                Some(p) => {
                    ...
                    match b {
                        // Only an empty *stream* reads `Empty` here (an
                        // absent child pair is never visited): vacuously
                        // disjoint.
                        IdNode::Empty => {}
                        // A present indexed node against a full `other`
                        // leaf: overlap.
                        IdNode::Full => return false,
                        // A full indexed leaf against a present `other`
                        // node: overlap.
                        IdNode::Internal { .. } if !al && !ar => return false,
                        // Both internal: descend over the child pairs
                        // `other` has, addressing the indexed side's
                        ...
                        IdNode::Internal {
                            ...
                            let (a_right, right_entry) = if al && ar {
                                // The node's entry is `entry` itself; its
                                // left subtree's entries follow it and
                                // keep targets strictly below the right
                                // child's position, so one partition
                                // finds the right subtree's entries.
            ...
            // The current pair closed disjoint: step into the innermost
            // queued right pair, or report the whole walk disjoint.
            match pending.pop() {
