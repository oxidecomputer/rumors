...
/// The board's exact cell count: 63 operation rows over 19 shapes (1197
/// combinations) minus the 225 where the shape's bundle supplies no
/// operand for the row's signature.
///
/// Derived per shape from the operand bundles. A version-only shape
/// (dense, bigroot, hugeleaf, cliff, harmonic) runs the 9 version-pair
/// rows (codec, compare, join/meet and their assign forms), its tick and
/// projection cells, the 4 linear-functional rows, the 2 rank rows, the
/// 3 text/hash rows, the causally row, the 11 clock rows, and its 8
/// rejection rows (the 5 version rejections and the 3 clock rejections):
/// 40 each. The id pair (parties only) runs the 10 party rows, the
/// adversarial-party tick row, its projection cell, the 11 clock rows,
/// and its 13 rejection rows (5 party rejections, `party_without_none`,
/// 3 clock rejections, and the 4 overlap rows its id source mints): 36.
/// The eleven cross shapes (comb-scatter and the ten tick-walk crosses:
...
/// carry a version, a mounted party pair, and a clock, so each runs the
/// version-only 40 plus the adversarial-party tick row, the 10 party
/// rows, and the 10 id-side rejection rows: 61 each. Scatter runs its 2
/// fold rows (its bundle carries fold operands alone, so no rejection
/// row applies); benign runs every row (63, both fold controls
/// included).
/// 40 x 5 + 36 + 61 x 11 + 2 + 63 = 972.
...
/// must move this pin.
const EXPECTED_CELLS: usize = 972;
