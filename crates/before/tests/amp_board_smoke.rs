...
/// The board's exact cell count: 65 operation rows over 21 shapes (1365
...
/// and projection cells, the 11 clock rows, and its 8 rejection rows (the
/// 5 version rejections and the 3 clock rejections): 42 each. The id pair
...
/// version-only 42 plus the adversarial-party tick row, the 10 party
/// rows, and the 10 id-side rejection rows: 63 each. Scatter runs its 2
/// fold rows (its bundle carries fold operands alone, so no rejection
/// row applies); benign runs every row (65, both fold controls
/// included).
/// 42 x 7 + 36 + 63 x 11 + 2 + 65 = 1090.
...
/// must move this pin.
const EXPECTED_CELLS: usize = 1090;
