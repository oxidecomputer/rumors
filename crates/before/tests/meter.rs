...
mod text_env {
    ...
    pub const SKYLINE_PARSE_DENSE: SweepEnvelope = sweep_envelope(4_041_052, 0, 625_007, 937_515, 375_003); //  3_232_857, 0, 1_500_013, 750_012; re-pinned to the new readings (limb 1_000_007, scan 750_012, heap 3_232_841): payload codes move as machine words and the accumulator's quick register folds narrow values without digit or limb work; re-pinned (limb 500_005, scan 750_012, heap 3_232_841): decoded payloads ride the word-valued form, so narrow-value work leaves the limb denomination (touch and scan floors stay the liveness signal)
    pub const SKYLINE_PARSE_BIGROOT: SweepEnvelope = sweep_envelope(377_944, 0, 52_357, 275_023, 31_413); //    302_363, 0,   121_899, 220_018; re-pinned to the new readings (limb 81_891, scan 220_018, heap 302_355): payload codes move as machine words and the accumulator's quick register folds narrow values without digit or limb work; re-pinned (limb 41_885, scan 220_018, heap 302_355): decoded payloads ride the word-valued form, so narrow-value work leaves the limb denomination (touch and scan floors stay the liveness signal)
    pub const SKYLINE_PARSE_HUGELEAF: SweepEnvelope  = sweep_envelope(   163_464,        0,     7_329,   625_005, 4_397); //    130_771, 0,     5_863, 500_004
    pub const SKYLINE_PARSE_CLIFF: SweepEnvelope = sweep_envelope(344_152, 0, 56_563, 35_845, 33_937); //    275_441, 0,    67_764, 28_676; re-pinned (limb 45_250, scan 28_676, heap 275_321): decoded payloads ride the word-valued form, so narrow-value work leaves the limb denomination (touch and scan floors stay the liveness signal)
}
