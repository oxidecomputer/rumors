...
//
// Honesty rule: instruction counts are a deterministic guest measure, so
// the widget presents shapes and ratios only — growth against a
// compensation hypothesis, quantile bands, spread — and never an absolute
// count. Axis gridlines carry no numbers; readouts are ratios.
...
class Widget {
  ...
  cdfLast(ly) {
    const col = this.data.cols[this.data.cols.length - 1];
    const li = this.sizes.length - 1;
    if (ly <= this.qref.min[li] + 1e-9) return 0;
    if (ly >= this.qref.max[li] - 1e-9) return 1;
    const total = col.c.reduce((a, b) => a + b, 0);
  ...
  setAnchorValue(lyRaw) {
    ...
    if (this.Y && this.lastShift !== undefined &&
        Math.abs(this.Y(ly - this.lastShift) - this.Y(med - this.lastShift)) < 3) ly = med;
    this.q = ly === med ? 0.5 : this.cdfLast(ly);
    this.update(false);
  }
  ...
  dragStart(ev) {
    ...
    const move = e => {
      ...
      if (this._dragMoved)
        this.setAnchorValue(this.Y.invert(py) + this.lastShift);
    };
    const up = () => {
  ...
  addGuideSilent(text) {
    const r = parseBound(text);
    if (r.err) return r.err;
    let l0 = null, l1 = null;
    try {
      for (const n of this.sizes) {
        const v = r.f(n);
        if (v.s <= 0 || !isFinite(v.l)) return "not positive at n=" + n;
        if (n === this.sizes[0]) l0 = v.l;
        if (n === this.sizes[this.sizes.length - 1]) l1 = v.l;
      }
    } catch (e) { return String(e); }
    // sort key: log-slope across the measured range (constants cancel),
    // tiebroken by magnitude at the largest n, then text
  ...
  shiftAt(n) {
    ...
    return this.guides.find(g => g.text === this.active).f(n).l;
  }
  ...
  buildDom(host) {
    ...
    prov.innerHTML = "commit <code>" + d.commit + "</code> \u00b7 seed <code>" + d.seed +
      "</code> \u00b7 avg <code>" + d.spc + "</code> samples/column";
    const measure = mk(this.footer, "span", "fs-prov");
    measure.textContent = "size measure: " + d.size_measure;
    const honesty = mk(this.footer, "span", "fs-prov");
    honesty.textContent =
      "instructions are a deterministic guest measure: compare shapes and ratios, not absolute counts";
  }
  ...
  buildStatic() {
    ...
    attrs(mk(filt, "feGaussianBlur"), { stdDeviation: "0 1.6" });
    ...
    attrs(this.ylabel, { transform: `translate(16,${M.t + PH / 2}) rotate(-90)`, "text-anchor": "middle" });
    ...
    // display bins: data-side kernel + SVG blur; quantiles use raw counts
    const R = 3, SIG = 1.0;
    const kern = [];
    ...
    for (let ci = 0; ci < this.data.cols.length; ci++) {
      ...
      const sm = new Array(col.c.length + 2 * R).fill(0);
      for (let j = 0; j < col.c.length; j++)
        for (let i = -R; i <= R; i++) sm[j + R + i] += col.c[j] * kern[i + R];
      const peak = Math.max(...sm);
      for (let j = 0; j < sm.length; j++) {
        const v = sm[j] / peak;
        if (v <= 0) continue;
        const vv = Math.max(v, 0.015);
        this.bins.push({ ci, vTop: (col.k0 - R + j + 1) * this.res, v: Math.pow(vv, 0.6) });
      }
      this.smExt.push([(col.k0 - R) * this.res, (col.k0 + col.c.length + R) * this.res]);
    }
    this.binEls = new Map();
    ...
    this.qLabel.setAttribute("x", W - M.r + SLX + 13);
  ...
  update(animate) {
    ...
    const li = this.sizes.length - 1;
    this.lastShift = shifts[li];
    const aI = li;                       // anchor at largest n in every view,
    const aN = this.sizes[aI];           // so guides always intersect the probe
    const anchorLy = tv.raw[aI];         // at the asymptotic end
    ...
    for (const g of this.guides) {
      ...
      if (!e) {
        ...
        poly.addEventListener("pointerdown", ev => { if (g.text === this.active) this.dragStart(ev); });
      ...
      e.title.textContent = (g.text === this.active ? "deselect " : "compensate by ") + g.text;
    }
    ...
    this.slCaption.setAttribute("x", W - M.r + SLX + 13);
...
const Fuelscape = {
  parse: parseBound,
  // Hydration is lazy, per island, on its expander's first opening: a
  // rustdoc type page carries every method's island, and each widget is
  // thousands of SVG nodes \u2014 building them all at page load would tax
  // every visit for expanders never opened. The `toggle` event fires for
  // programmatic `open` changes too, so rustdoc's expand-all control
  // still hydrates what it reveals.
  hydrate(scope) {
